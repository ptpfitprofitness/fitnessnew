...
<?php echo $this->Form->create('Member' ,array('controller'=>'members', 'action'=>'index', 'enctype'=>'multipart/form-data', 'class'=>'mainForm', 'id'=>'valid')); ?>
...
<div class="table">
    <div class="head">
		<h5 class="iFrames">Manage Customers</h5>
		<div class="rowElem noborder" style="clear:none;margin-top: -4px;">
			<?php if(isset($members) && count($members) > 0){ ?>
				<?php  echo $this->Form->select('Member.statusTop',unserialize($config['status_array']),array('empty'=>'Select','class'=>'topAction','style'=>'width:20%')); ?>&nbsp;
				<?php  echo $this->Form->submit('OK', array('style'=>'float:none;margin-left: -1px;','class'=>'blueBtn submitForm','name'=>'data[Member][submit]', 'div'=>false, 'onclick'=>"return del('data[Member][id][]')",'id'=>'StatusTop'));?>				
				
			<?php 	} ?> 		
				<a href="<?php echo $this->Html->url(array('controller'=>'members', 'action'=>'add')); ?>" style="float: right; margin-top:0px; padding: 2px 13px;margin-right:0px;" class='blueBtn'>Add New</a>
		</div>		
	</div>
        <div class="dataTables_wrapper" id="example_wrapper">
			<?php echo $this->Form->create('Member',array('controller'=>'members','action'=>'index/'.$this->params["pass"][0],'class'=>'mainForm')); ?>	
				<div class="">
					<div class="dataTables_filter" id="example_filter" style="right:15%;top:-38px;">
						<label>Search: <input type="text" name="keyword" value="<?php echo $keyword;?>" placeholder="Search by email or UserName..."/>
			...
			<?php 
				...
				if(isset($members) && count($members) > 0){
					$this->Paginator->options(array('url' => array("controller"=>"members","action"=>"index/$tab/keyword:$keyword")));
			...
			<table cellspacing="0" cellpadding="0" border="0" id="" class="display">
				...
				if(isset($members) && count($members) > 0){
					...
					if(trim($prow['Member']['full_name']))
						$full = "(".trim($prow['Member']['full_name']).")";
					else
						$full = "";	
				?>
				...
				<tr class="gradeA <?php if($count%2==0) echo 'odd'; else echo 'even';?>">
					...
					<td><a href="<?php echo $config['url']; ?>admin/members/view/<?php echo $prow['Member']['id']; ?>" target="_blank" ><?php echo $prow['Member']['username']."<br>".$full; ?></a></td>
					<td><?php echo $prow['Member']['email'] ?></td>		
										
					<td align="center"><?php echo date($config['date_format'], strtotime($prow['Member']['created'])); ?></td>
					<td align="center"><?php 
						if($prow['Member']['status']==1){
					...
					<td align="center"><?php echo $this->Html->link($this->Html->image('edit-icon.png'),array('controller'=>'members','action'=>'edit/'.$prow['Member']['id'] ), array('title'=>'Edit Member','escape'=>false));?>
					</td>
				...
				<?php if(isset($members) && count($members) > 0){?>
					...
					<div class="rowElem noborder">
						<?php  //echo $this->Form->select('Member.status',unserialize($config['status_array']),array('empty'=>'Select', 'id'=>'Status','style'=>'width:20%;float:left;')); ?>&nbsp;
						<?php  //echo $this->Form->submit('OK', array('style'=>'float:left;','class'=>'blueBtn submitForm','name'=>'data[Member][submit]', 'div'=>false, 'onclick'=>"return del('data[Member][id][]')"));?>
					</div>
					</td></tr>
				<?php }?>
				</tbody>
