...
##  Description :		Admin Trainee Index
## *****************************************************************
...
<?php echo $this->Form->create('Coupon' ,array('controller'=>'coupons', 'action'=>'index', 'enctype'=>'multipart/form-data', 'class'=>'mainForm', 'id'=>'valid')); ?>
...
<div class="table">
    <div class="head">
		<h5 class="iFrames">Manage Coupons</h5>
		<div class="rowElem noborder" style="clear:none;margin-top: -4px;">
			<?php if(isset($coupons) && count($coupons) > 0){ ?>
				<?php  echo $this->Form->select('Coupon.statusTop',unserialize($config['status_array']),array('empty'=>'Select','class'=>'topAction','style'=>'width:20%')); ?>&nbsp;
				<?php  echo $this->Form->submit('OK', array('style'=>'float:none;margin-left: -1px;','class'=>'blueBtn submitForm','name'=>'data[Coupon][submit]', 'div'=>false, 'onclick'=>"return del('data[Coupon][id][]')",'id'=>'StatusTop'));?>				
				
			<?php 	} ?> 		
				<a href="<?php echo $this->Html->url(array('controller'=>'coupons', 'action'=>'add')); ?>" style="float: right; margin-top:0px; padding: 2px 13px;margin-right:0px;" class='blueBtn'>Add New</a>
		</div>		
	</div>
        <div class="dataTables_wrapper" id="example_wrapper">
			<?php echo $this->Form->create('Coupon',array('controller'=>'coupons','action'=>'index/'.$this->params["pass"][0],'class'=>'mainForm')); ?>	
				<div class="">
					<div class="dataTables_filter" id="example_filter" style="right:15%;top:-38px;">
						<label>Search: <input type="text" name="keyword" value="<?php echo $keyword;?>" placeholder="Search by content..."/>
			...
			<?php 
				...
				if(isset($coupons) && count($coupons) > 0){
					$this->Paginator->options(array('url' => array("controller"=>"coupons","action"=>"index/$tab/keyword:$keyword")));
			...
			<table cellspacing="0" cellpadding="0" border="0" id="" class="display">
                <thead>
                    <tr>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
							<div style="padding-right:8px;"><?php echo $this->Form->text('Coupon.all', array('type'=>'checkbox', 'id'=>'data[Coupon][all]', 'onclick'=>"selectDeselect('data[Coupon][id][]', this.name);"));?></div>
						</th>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
				...
				<tr class="gradeA <?php if($count%2==0) echo 'odd'; else echo 'even';?>">
					<td align="center"><?php echo $this->Form->text('Coupon.id][', array('type'=>'checkbox', 'value'=>$prow['Coupon']['id'],  'onclick'=>"checkSelection('data[Coupon][id][]', 'data[Coupon][all]')"));?></td>
					<td align="center"><?php echo $index;?></td>
					
					<td><a href="<?php echo $config['url']; ?>admin/Coupons/view/<?php echo $prow['Coupon']['id']; ?>" target="_blank" ><?php echo $prow['Coupon']['coupon_name'] ?></a></td>	

					<td><a href="<?php echo $config['url']; ?>admin/Coupons/view/<?php echo $prow['Coupon']['id']; ?>" target="_blank" ><?php echo $prow['Coupon']['coupon_code'] ?></a></td>		

					<td align="center">
					<?php 
						if($prow['Coupon']['discount_type']=='Fixed'){
							echo "Fixed Value";	
						}else{
					...
					<td align="center"><?php echo date($config['date_format'], strtotime($prow['Coupon']['start_date'])); ?></td>
					
					<td align="center"><?php echo date($config['date_format'], strtotime($prow['Coupon']['expiry_date'])); ?></td>
					
					<td align="center">
					<?php 
						if($prow['Coupon']['status']==1){
					...
					<td align="center"><?php echo $this->Html->link($this->Html->image('edit-icon.png'),array('controller'=>'coupons','action'=>'edit/'.$prow['Coupon']['id'] ), array('title'=>'Edit Coupon','escape'=>false));?>
					</td>
