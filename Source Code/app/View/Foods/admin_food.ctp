...
<?php echo $this->Form->create(null,array('controller'=>'foods', 'action'=>'food', 'enctype'=>'multipart/form-data', 'class'=>'mainForm', 'id'=>'valid')); ?>
...
<div class="table">
    <div class="head">
		<h5 class="iFrames">Manage Food Nutrition Log</h5>
		<div class="rowElem noborder" style="clear:none;margin-top: -4px;">
			<?php if(isset($branches) && count($branches) > 0){ ?>
				<?php  echo $this->Form->select('FoodNutritionLog.statusTop',unserialize($config['status_array']),array('empty'=>'Select','class'=>'topAction','style'=>'width:20%')); ?>&nbsp;
				<?php  echo $this->Form->submit('OK', array('style'=>'float:none;margin-left: -1px;','class'=>'blueBtn submitForm','name'=>'data[FoodNutritionLog][submit]', 'div'=>false, 'onclick'=>"return del('data[FoodNutritionLog][id][]')",'id'=>'StatusTop'));?>				
				
			<?php 	} ?> 	
				
			
				<a href="<?php echo $this->Html->url(array('controller'=>'foods', 'action'=>'addfood')); ?>" style="float: right; margin-top:0px; padding: 2px 13px;margin-right:0px;" class='blueBtn'>Add New</a>
		</div>		
		
	</div>
	
        <div class="dataTables_wrapper" id="example_wrapper">
			<?php echo $this->Form->create('FoodNutritionLog',array('controller'=>'foods','action'=>'food/'.$this->params["pass"][0],'class'=>'mainForm')); ?>	
				<div class="">
					<div class="dataTables_filter" id="example_filter" style="right:15%;top:-38px;">
						<label>Search: <input type="text" name="keyword" value="<?php echo $keyword;?>" placeholder="Search by type or Name..."/>
			...
			<?php 
				...
				if(isset($branches) && count($branches) > 0){
					$this->Paginator->options(array('url' => array("controller"=>"foods","action"=>"food/$tab/keyword:$keyword")));
					echo $this->element('admin/paginate');
				} 
			?>
			<br/>
			<div><a href="<?php echo $this->Html->url(array('controller'=>'foods', 'action'=>'usdaimport')); ?>" style="float: right; margin-bottom:10px; padding: 2px 13px;margin-right:10px;" class='blueBtn'>Import Usda Data</a></div>
			<br/>
			<table cellspacing="0" cellpadding="0" border="0" id="" class="display">
                <thead>
                    <tr>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
							<div style="padding-right:8px;"><?php echo $this->Form->text('FoodNutritionLog.all', array('type'=>'checkbox', 'id'=>'data[FoodNutritionLog][all]', 'onclick'=>"selectDeselect('data[FoodNutritionLog][id][]', this.name);"));?></div>
						</th>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
				...
				if(isset($branches) && count($branches) > 0){
					...
					if(trim($prow['FoodNutritionLog']['name']))
						$full = "(".trim($prow['FoodNutritionLog']['name']).")";
					else
						$full = "";	
				?>
				...
				<tr class="gradeA <?php if($count%2==0) echo 'odd'; else echo 'even';?>">
					<td align="center"><?php echo $this->Form->text('FoodNutritionLog.id][', array('type'=>'checkbox', 'value'=>$prow['FoodNutritionLog']['id'],  'onclick'=>"checkSelection('data[FoodNutritionLog][id][]', 'data[FoodNutritionLog][all]')"));?></td>
					<td align="center"><?php echo $index;?></td>
					<td><a href="<?php echo $config['url']; ?>admin/foods/viewfood/<?php echo $prow['FoodNutritionLog']['id']; ?>" target="_blank" ><?php echo $prow['FoodNutritionLog']['type']."<br>".$full; ?></a></td>
					<td><?php echo $prow['FoodNutritionLog']['name'] ?></td>		
										
					<td align="center"><?php echo date($config['date_format'], strtotime($prow['FoodNutritionLog']['added_date'])); ?></td>
					<td align="center"><?php 
						if($prow['FoodNutritionLog']['status']==1){
					...
					<td align="center"><?php echo $this->Html->link($this->Html->image('edit-icon.png'),array('controller'=>'foods','action'=>'editfood/'.$prow['FoodNutritionLog']['id'] ), array('title'=>'Edit Food','escape'=>false));?>
					</td>
