    ...
    <div class="row">
     <?php echo $this->element('lefttrainer');?>
      <div class="eight inside-head columns">
        ...
        <div>
        <h2>Edit Group</h2>
        <form accept-charset="utf-8" method="post" id="validaddgroup" class="resform-wrap" enctype="multipart/form-data" controller="home" action="/home/editgroup/<?php echo base64_encode($gid);?>">
		   
         <input type="hidden"  name="data[Group][id]" id="GroupId" value="<?php echo $gid;?>"/> 
		 
        <input type="hidden" id="GroupTrainerId" value=" <?php echo $setSpecalistArr[$utype]['id'];?>"  name="data[Group][trainer_id]"/>
        
		<div class="twelve columns">
		
        <?php echo $this->Form->text('Group.group_name', array('maxlength'=>255,'id'=>'groupname','class'=>'validate[required]','placeholder'=>'Group Name')); ?>
		...
		<?php  echo $this->Form->select('Group.status',$notify,array('empty'=>'-- Select Status --','class'=>'sltbx','onchange'=>'document.getElementById(\'customSelectstatus\').value= this.options[this.selectedIndex].text')); ?>
			<input type="text" id="customSelectstatus" value="<?php if($this->data['Group']['status']!=''){
             foreach($notify as $key=>$val)
				{
                  if($key==$this->data['Group']['status'])
                  {
                  	 echo $val;
                  }	
