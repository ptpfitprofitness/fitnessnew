...
{
	var website_url ='<?php echo $config['url']?>home/printAndSaveGroup/';
	$.ajax({
		type: "POST",
		url: website_url,
		...
		success: function(e)
		{
			...
			if( response.responseclassName == "nSuccess" ) {
				...
				document.location.href='<?php echo $config['url']?>home/print_exercise_history_group/<?php echo $gid;?>/<?php echo $schid;?>';	
...
<form id="editRWa" action="" method="POST" onsubmit="return validatefrmEW();">
        ...
        <div class="twelve already-member columns"> <h2>Print Group Workout</h2> 
