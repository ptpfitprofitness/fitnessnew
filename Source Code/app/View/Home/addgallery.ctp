     ...
     <?php echo $this->element('lefttrainer');?>
      <div class="eight inside-head columns">
        <ul class="profile-tabs-list desktop-tabs clearfix">
          <li><a href="#Profile" class="active"><span class="profile-ico9"><img src="<?php echo $config['url'];?>images/calender_ico.png"></span>Add Gallery</a></li>
        
        </ul>    
       
        <div>
        <h2>Add Gallery</h2>
        <form accept-charset="utf-8" method="post" id="validclubtrainer" class="resform-wrap" enctype="multipart/form-data" controller="home" action="<?php echo $config['url'];?>home/addgallery/">
		
        
               <input type="hidden" id="TrainerId" value=" <?php echo $setSpecalistArr[$utype]['id'];?>"  name="data[Gallery][trainer_id]"/>
        
		
        
            <?php echo $this->Form->text('Gallery.gallery_titile', array('maxlength'=>255,'id'=>'gallery_titile', 'class'=>'validate[required]','placeholder'=>'Gallery Title','style=width:66%;')); ?>
			<?php echo $this->Form->input('Gallery.gallery_image', array('type' => 'file')); ?>  
			<p>Please upload a image with 800x600 pixels</p>
            <input type="submit" class="submit-nav" name="submit" value="Save"  style="margin-left:20px;"/>
          </form>
        </div>
