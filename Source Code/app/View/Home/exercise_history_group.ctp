...
?>
<style type="text/css">
#validcertificationtrainers .prev {float:left;font-size: 15px; font-weight: bold; }
#validcertificationtrainers .next {float:right;font-size: 15px; font-weight: bold; margin: 0 -70px 0 0; }
#validcertificationtrainers .pagination-div{width:132%;}
</style>
<?php
/*echo '<pre>';
		   		...
		   		success: function(e)
					{
						...
						alert(response.errorMsg);
						//document.location.href=document.location.href;
						document.location.href='<?php echo $config['url'].'home/scheduling_calendar';?>';
 ...
 function creatework(str)
 {
 	if(str=='c') 
 	{
 	  if($("#GroupId").val()!='')	
 	  {
 	  	popupClose('popFdn');
 	  	//popupOpen('popFd');
		var stv='<?php echo $selectedslt;?>';
 	  	$('#a').show();
 		$('#b').show();
 		$('#c').show();
 		$('#d').show();
 		$('#k').hide();	
 		var clvid=$("#GroupId").val();
 		 window.open("http://www.ptpfitpro.com/home/exercise_history_build_group/"+clvid+'/'+stv, "_blank", "fullscreen=no, height=500, toolbar=yes, scrollbars=yes, resizable=yes, top=200, left=200, width=800");
 	  }
 	  else
 	  {
...
function redirectionExerhis()
{
	popupClose('popFdn');
	document.location.href='<?php echo $config['url']?>home/scheduling_calendar/<?php echo $msdate1; ?>';
...
function getCatvalue(str)
{
	var website_url ='<?php echo $config['url']?>home/getcatval';
	$.ajax({
		type: "GET",
		url: website_url,				
		success: function(e)
		{
			var response = eval(' ( '+e+' ) ');
			if( response.responseclassName == "nSuccess" ) {
					$('#workoutcategory1').html(response.responseval);
					$('#workoutcategory').html(response.responseval);
					if(str=='d')
					{
					popupClose('popFdnAsVw');
					popupOpen('popSW');
					} else
					{
						popupClose('popFdnAsVw');
     ...
     <?php //echo $this->element('lefttrainer');?>
      <div class="eight inside-head columns">
        ...
        <ul class="profile-tabs-list mobile-tab clearfix">
           ...
           <?php /*if($showoff==1){*/?>
           <input type="button" name="submit" value="Build a Group Workout" class="change-pic-nav" onclick="popupOpen3n('popFd');" style="  float: left; height: 36px; margin: 0 0 10px 10px; width: 156px;"/>
 ...
 <form accept-charset="utf-8" method="post" id="validcertificationtrainers" class="resform-wrap" enctype="multipart/form-data" controller="home" action="/home/addcertification_trainer/">
		...
		<input type="hidden"  name="data[CertificationTrainers][trainer_id]" id="CertificationTrainers_trainer_id" value="<?php echo $setSpecalistArr[$utype]['id'];?>"/>
            ...
            <div class="row">
              ...
              echo $this->Form->select('Group.id',$groupname,array('empty'=>'-- Select Group --','class'=>'sltbx','onchange'=>'document.getElementById(\'customSelectid\').value= this.options[this.selectedIndex].text; chngurl(this.value);')); ?>          
                ...
                }else{echo '-- Select Group--';}?>"/>
                
              </div>
             
              <script>
              document.getElementById('GroupId').value='<?php echo $clientid;?>';
              </script>
  ...
  <div class="clear"></div>
		...
		<a onclick="addcate('w');" style="float:right;">Add New Category</a>
...
<!-- Exercise History Popup  -->
                <div id="popFdn" class="main-popup">
                  <div class="overlaybox common-overlay"></div>
                  <div id="thirtydays" class="register-form-popup common-overlaycontent" style="width:684px;margin-left:-305px;"> 
				 <!-- <a class="close-nav" onclick="popupClose('popFdn');" id="pop4n" href="javascript:void(0);"></a>-->
				  <a class="close-nav" onclick="redirectionExerhis();" id="pop4n" href="javascript:void(0);"></a>
				  
                    <div class="row register-popup-form">
                      <div class="twelve field-pad columns"> 
                      
                       <h2>Build a Group Workout</h2>
                                           
						<table width="100%" border="1">
							<tr class="slectedmn">
								<td class="th2" colspan="3"><span style="float: left; line-height: 36px;"><input type="radio" name="Stype" id="Stype" value="c" onclick="creatework(this.value);"> : Continue With The Selected Group</span> </td><!--<td><span style="float: left; line-height: 36px;"><input type="radio" name="Stype" id="Stype" value="w" onclick="creatework(this.value);"> : Continue Without Selected Group</span></td>	-->		
							</tr>
						</table>                       
                      </div>                    
                    </div>
		...
		<a onclick="addcate('d');" style="float:right;">Add New Category</a>
                       ...
                       <form id="addworkcat" action="" method="POST" onsubmit="return validateaddworkcat();">
						...
						<div class="row">
						  <div class="twelve columns">
						     ...
						     <input type="hidden" id="rfd" name="rfd" value="" />
