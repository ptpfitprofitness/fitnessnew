     ...
     <?php echo $this->element('lefttrainer');?>
      <div class="eight inside-head columns">
        <ul class="profile-tabs-list desktop-tabs clearfix">
          <li><a href="#Profile" class="active"><span class="profile-ico9"><img src="<?php echo $config['url'];?>images/calender_ico.png"></span>Manage Session</a></li>
        
        </ul>    
       
        <div>
        <h2>Edit Session Type</h2>
        <form accept-charset="utf-8" method="post" id="validclubtrainer" class="resform-wrap" enctype="multipart/form-data" controller="home" action="<?php echo $config['url'];?>home/editworkout/<?php echo base64_encode($bid);?>">
		
       <input type="hidden"  name="data[WorkOuts][id]" id="WorkOutsId" value="<?php echo $bid;?>"/> 
        
               <input type="hidden" id="TrainerId" value=" <?php echo $setSpecalistArr[$utype]['id'];?>"  name="data[WorkOuts][trainer_id]"/>
        
		
        
            <?php echo $this->Form->text('WorkOuts.workout_name', array('maxlength'=>255,'id'=>'workout_name', 'class'=>'validate[required]','placeholder'=>'Session Type','style=width:66%;')); ?>

				<?php echo $this->Form->error('WorkOuts.workout_name', null, array('class' => 'error')); ?>
          

				<?php echo $this->Form->text('WorkOuts.workout_time', array('maxlength'=>255,'id'=>'workout_time', 'class'=>'validate[required,custom[number]','placeholder'=>'Session Time Example 15,30,45...etc Mins','style=width:66%;')); ?>

				<?php echo $this->Form->error('WorkOuts.workout_time', null, array('class' => 'error')); ?>
            
             <?php //echo $this->Form->text('WorkOuts.workout_price', array('maxlength'=>20,'id'=>'workout_price', 'class'=>'validate[required,custom[number]','placeholder'=>'Price 15,30,45...etc ' , 'style=width:66%;')); ?>
             <?php echo $this->Form->text('WorkOuts.workout_price', array('maxlength'=>20,'id'=>'workout_price', 'class'=>'','placeholder'=>'Price 15,30,45...etc ' , 'style=width:66%;')); ?>

				<?php echo $this->Form->error('WorkOuts.workout_price', null, array('class' => 'error')); ?>
			...
			<div class="row">
              <div class="twelve form-select columns" style="width:66%;">
             
				
   <?php $notify=array('0'=>'InActive','1'=>'Active');
 ?>
               ?>       
				<?php  echo $this->Form->select('WorkOuts.status',$notify,array('empty'=>'-- Select Status --','class'=>'sltbx','onchange'=>'document.getElementById(\'customSelectClubid1\').value= this.options[this.selectedIndex].text')); ?>
				
				      
			
                <input type="text" id="customSelectClubid1" value="<?php if($this->data['WorkOuts']['status']!=''){
                foreach($notify as $key=>$val)
                {
                  if($key==$this->data['WorkOuts']['status'])
                  {
                  	 echo $val;
                  }	
                	
                }
                }else{echo '-- Select Status --';}?>"/>
