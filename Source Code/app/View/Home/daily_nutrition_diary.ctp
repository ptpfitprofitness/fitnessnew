...
function edittrainer(str)
{
   ...
   if(str!='')
   {
   	document.location.href="<?php echo $config['url'];?>trainees/editbranch/"+str;
   	
   }

}

function savenutition(str)
{

   if(str!='')
   {
   	document.location.href="<?php echo $config['url'];?>home/add_daily_nutrition_diary/"+str;
...
function newtrainee()
{
	...
	document.location.href="<?php echo $config['url'];?>trainees/addbranch/";
}
         ...
         //pr($clients);
          ...
          ?>
            <li>
              ...
              <div class="list-colum second-tabs"><?php if(!empty($nutrition['Trainee']['username'])){echo $nutrition['Trainee']['username']; }; ?></div>
              <div class="list-colum third-tabs"><?php if(!empty($nutrition['Trainee']['email'])){echo  $nutrition['Trainee']['email']; };  ?></div>
              
              <div class="list-colum four-tabs"><input type="button" style="width:120px;" class="change-pic-nav" onclick="savenutition('<?php echo base64_encode($nutrition['Trainee']['id']);?>');" value="Add Food Log" name="submit"></div>
            </li>
         <?php   $cnt++; ?>
            <?php  }?>
