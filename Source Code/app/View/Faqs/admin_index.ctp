...
<?php echo $this->Form->create('Faq' ,array('controller'=>'faqs', 'action'=>'index', 'enctype'=>'multipart/form-data', 'class'=>'mainForm', 'id'=>'valid')); ?>
...
<div class="table">
    <div class="head">
		<h5 class="iFrames">Manage Faqs</h5>
		<div class="rowElem noborder" style="clear:none;margin-top: -4px;">
			<?php if(isset($faqs) && count($faqs) > 0){ ?>
				<?php  echo $this->Form->select('Faq.statusTop',unserialize($config['status_array']),array('empty'=>'Select','class'=>'topAction','style'=>'width:20%')); ?>&nbsp;
				<?php  echo $this->Form->submit('OK', array('style'=>'float:none;margin-left: -1px;','class'=>'blueBtn submitForm','name'=>'data[Faq][submit]', 'div'=>false, 'onclick'=>"return del('data[Faq][id][]')",'id'=>'StatusTop'));?>				
				
			<?php 	} ?> 		
				<a href="<?php echo $this->Html->url(array('controller'=>'faqs', 'action'=>'add')); ?>" style="float: right; margin-top:0px; padding: 2px 13px;margin-right:0px;" class='blueBtn'>Add New</a>
		</div>		
	</div>
        <div class="dataTables_wrapper" id="example_wrapper">
			<?php echo $this->Form->create('Faq',array('controller'=>'faqs','action'=>'index/'.$this->params["pass"][0],'class'=>'mainForm')); ?>	
				<div class="">
			...
			<?php 
				...
				if(isset($trainees) && count($trainees) > 0){
					$this->Paginator->options(array('url' => array("controller"=>"faqs","action"=>"index/$tab/keyword:$keyword")));
			...
			<table cellspacing="0" cellpadding="0" border="0" id="" class="display">
                <thead>
                    <tr>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
							<div style="padding-right:8px;"><?php echo $this->Form->text('Faq.all', array('type'=>'checkbox', 'id'=>'data[Faq][all]', 'onclick'=>"selectDeselect('data[Faq][id][]', this.name);"));?></div>
						</th>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
				...
				<tr class="gradeA <?php if($count%2==0) echo 'odd'; else echo 'even';?>">
					<td align="center"><?php echo $this->Form->text('Faq.id][', array('type'=>'checkbox', 'value'=>$prow['Faq']['id'],  'onclick'=>"checkSelection('data[Faq][id][]', 'data[Faq][all]')"));?></td>
					<td align="center"><?php echo $index;?></td>
					<td><a href="<?php echo $config['url']; ?>admin/Faqs/view/<?php echo $prow['Faq']['id']; ?>" target="_blank" ><?php echo $prow['Faq']['question'] ?></a></td>					
										
					<td align="center"><?php echo date($config['date_format'], strtotime($prow['Faq']['added_date'])); ?></td>
					<td align="center"><?php 
						if($prow['Faq']['status']==1){
					...
					<td align="center"><?php echo $this->Html->link($this->Html->image('edit-icon.png'),array('controller'=>'faqs','action'=>'edit/'.$prow['Faq']['id'] ), array('title'=>'Edit Faq','escape'=>false));?>
					</td>
