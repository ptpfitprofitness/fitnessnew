...
<?php echo $this->Form->create('Managemail' ,array('controller'=>'Managemails', 'action'=>'index', 'enctype'=>'multipart/form-data', 'class'=>'mainForm', 'id'=>'valid')); ?>
...
<div class="table">
    <div class="head">
		<h5 class="iFrames">Manage Mails</h5>
		<div class="rowElem noborder" style="clear:none;margin-top: -4px;">
			<?php if(isset($Managemails) && count($Managemails) > 0){ ?>
				<?php  echo $this->Form->select('Managemail.statusTop',unserialize($config['status_array']),array('empty'=>'Select','class'=>'topAction','style'=>'width:20%')); ?>&nbsp;
				<?php  echo $this->Form->submit('OK', array('style'=>'float:none;margin-left: -1px;','class'=>'blueBtn submitForm','name'=>'data[Managemail][submit]', 'div'=>false, 'onclick'=>"return del('data[Managemail][id][]')",'id'=>'StatusTop'));?>				
				
			<?php 	} ?> 		
				<!--<a href="<?php //echo $this->Html->url(array('controller'=>'Managemails', 'action'=>'add')); ?>" style="float: right; margin-top:0px; padding: 2px 13px;margin-right:0px;" class='blueBtn'>Add New</a>-->
		</div>		
	</div>
        <div class="dataTables_wrapper" id="example_wrapper">
			<?php echo $this->Form->create('Managemail',array('controller'=>'Managemails','action'=>'index/'.$this->params["pass"][0],'class'=>'mainForm')); ?>	
				<div class="">
			...
			<?php 
				...
				if(isset($trainees) && count($trainees) > 0){
					$this->Paginator->options(array('url' => array("controller"=>"Managemails","action"=>"index/$tab/keyword:$keyword")));
			...
			<table cellspacing="0" cellpadding="0" border="0" id="" class="display">
                <thead>
                    <tr>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
							<div style="padding-right:8px;"><?php echo $this->Form->text('Managemail.all', array('type'=>'checkbox', 'id'=>'data[Managemail][all]', 'onclick'=>"selectDeselect('data[Managemail][id][]', this.name);"));?></div>
						</th>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
				...
				<?php
				if(isset($Managemails) && count($Managemails) > 0){
					$count = 0; 
					foreach($Managemails as $prow){
				...
				<tr class="gradeA <?php if($count%2==0) echo 'odd'; else echo 'even';?>">
					<td align="center"><?php echo $this->Form->text('Managemail.id][', array('type'=>'checkbox', 'value'=>$prow['Managemail']['id'],  'onclick'=>"checkSelection('data[Managemail][id][]', 'data[Managemail][all]')"));?></td>
					<td align="center"><?php echo $index;?></td>
					<td><?php echo $prow['Managemail']['mails_type'] ?></td>
					
					<td><?php echo $prow['Managemail']['subject'] ?></td>	
					<td><?php echo $prow['Managemail']['mail_fire_events'] ?></td>						
										
					<!--<td align="center"><?php //echo date($config['date_format'], strtotime($prow['Managemail']['added_date'])); ?></td>-->
					<td align="center"><?php 
						if($prow['Managemail']['status']==1){
					...
					<td align="center"><?php echo $this->Html->link($this->Html->image('edit-icon.png'),array('controller'=>'Managemails','action'=>'edit/'.$prow['Managemail']['id'] ), array('title'=>'Edit Mails','escape'=>false));?>
					</td>
