...
<?php echo $this->Form->create(null ,array('controller'=>'news', 'action'=>'index', 'enctype'=>'multipart/form-data', 'class'=>'mainForm', 'id'=>'valid')); ?>
    ...
    <div class="head">
		...
		<div class="rowElem noborder" style="clear:none;margin-top: -4px;">
			<?php if(isset($news) && count($news) > 0){ ?>
				<?php  echo $this->Form->select('Neww.statusTop',unserialize($config['status_array']),array('empty'=>'Select','class'=>'topAction','style'=>'width:20%')); ?>&nbsp;
				<?php  echo $this->Form->submit('OK', array('style'=>'float:none;margin-left: -1px;','class'=>'blueBtn submitForm','name'=>'data[Neww][submit]', 'div'=>false, 'onclick'=>"return del('data[Neww][id][]')",'id'=>'StatusTop'));?>				
				
			<?php 	} ?> 		
				<a href="<?php echo $this->Html->url(array('controller'=>'news', 'action'=>'add')); ?>" style="float: right; margin-top:0px; padding: 2px 13px;margin-right:0px;" class='blueBtn'>Add New</a>
		</div>		
	</div>
        <div class="dataTables_wrapper" id="example_wrapper">
			<?php echo $this->Form->create('Neww',array('controller'=>'news','action'=>'index/'.$this->params["pass"][0],'class'=>'mainForm')); ?>	
				<div class="">
			...
			<?php 
				...
				if(isset($news) && count($news) > 0){
					$this->Paginator->options(array('url' => array("controller"=>"news","action"=>"index/$tab/keyword:$keyword")));
			...
			<table cellspacing="0" cellpadding="0" border="0" id="" class="display">
                <thead>
                    <tr>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
							<div style="padding-right:8px;"><?php echo $this->Form->text('Neww.all', array('type'=>'checkbox', 'id'=>'data[Neww][all]', 'onclick'=>"selectDeselect('data[Neww][id][]', this.name);"));?></div>
						</th>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
				...
				<tr class="gradeA <?php if($count%2==0) echo 'odd'; else echo 'even';?>">
					<td align="center"><?php echo $this->Form->text('Neww.id][', array('type'=>'checkbox', 'value'=>$prow['Neww']['id'],  'onclick'=>"checkSelection('data[Neww][id][]', 'data[Neww][all]')"));?></td>
					<td align="center"><?php echo $index;?></td>
					<td><a href="<?php echo $config['url']; ?>admin/News/view/<?php echo $prow['Neww']['id']; ?>" target="_blank" ><?php echo $prow['Neww']['heading'] ?></a></td>
					
										
					<td align="center"><?php echo date($config['date_format'], strtotime($prow['Neww']['added_date'])); ?></td>
					<td align="center"><?php 
						if($prow['Neww']['status']==1){
					...
					<td align="center"><?php echo $this->Html->link($this->Html->image('edit-icon.png'),array('controller'=>'news','action'=>'edit/'.$prow['Neww']['id'] ), array('title'=>'Edit News','escape'=>false));?>
					</td>
