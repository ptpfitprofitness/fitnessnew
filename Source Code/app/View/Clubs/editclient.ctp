...
function removePic(elem) {
	...
	if(r){
		...
		$.ajax({
				url:"<?php echo $config['url'];?>trainee/removePic/",
				type:"POST",
				data:{id:elem.id},
				success:function(e) {
					var response = eval(' ( '+e+' ) ');
    ...
    <div class="row">
     <?php echo $this->element('leftclub');?>
      <div class="eight inside-head columns">
        ...
        <div>
        <h2>Edit Client</h2>
        <form accept-charset="utf-8" method="post" id="validclubtrainer" class="resform-wrap" enctype="multipart/form-data" controller="clubs" action="/clubs/editclient/">
		
        
        <input type="hidden" id="TraineeClubId" value=" <?php echo $setSpecalistArr['Club']['id'];?>"  name="data[Trainee][club_id]"/>
            ...
            <?php } else {?>  <input type="hidden" id="TrainerClubBranchId" value="<?php echo $setSpecalistArr[$utype]['id'];?>"  name="data[Trainee][club_branch_id]"/><?php }?>
            
            <div class="row">
              <div class="twelve form-select columns">
               
              	<?php  echo $this->Form->select('Trainee.trainer_id',$trainers,array('empty'=>'-- Select Trainer --','class'=>'sltbx','onchange'=>'document.getElementById(\'customSelectOrgid\').value= this.options[this.selectedIndex].text')); ?>     
                
			
                <input type="text" id="customSelectOrgid" value="<?php if($this->data['Trainee']['trainer_id']!=''){
                foreach($trainers as $key=>$val)
                {
                  if($key==$this->data['Trainee']['trainer_id'])
                  {
                  	 echo $val;
                  }	
                	
                }
                }else{echo '-- Select Trainer --';}?>"/>
              </div>
            </div>
