	...
	if(r){
		...
		$.ajax({
				...
				success:function(e) {
					...
					window.location.assign("<?php echo $config['url'];?>clubs/index");	
				}
		});
	}
}
...
<script>
		jQuery(document).ready( function() {
			...
			jQuery("#valideditingprofileform").validationEngine();
		});
	</script>
<style>
      ...
      <div class="eight inside-head columns">
        ...
        <ul class="profile-tabs-list mobile-tab clearfix">
                  ...
                  <div id="thirtydays" class="register-form-popup common-overlaycontent"> <a class="close-nav" onclick="popupClose('pop4');" id="pop4" href="javascript:void(0);"></a>
                    <div class="row register-popup-form">
                      <div class="twelve field-pad columns">
                        <form action="/clubs/uploadpic/" controller="home" enctype="multipart/form-data" class="resform-wrap" id="valid" method="post" accept-charset="utf-8" onsubmit="return validuppic();">
          ...
          <div>
            ...
            <form accept-charset="utf-8" method="post" id="valideditingprofileform" class="resform-wrap" enctype="multipart/form-data" controller="clubs" action="/clubs/editprofile/">
		<?php echo $this->Form->hidden('Club.id'); ?>
            
                       
         <?php //echo $this->Form->text('Club.username', array('maxlength'=>255,'id'=>'Username','readonly'=>'readonly', 'class'=>'validate[required]','placeholder'=>'Username')); ?>

				<?php //echo $this->Form->error('Club.username', null, array('class' => 'error')); ?>
           <?php echo $this->Form->text('Club.email', array('maxlength'=>255,'id'=>'EmailAddress','readonly'=>'readonly', 'class'=>'validate[required,custom[email],ajax[agantEmailValidate]]','placeholder'=>'Email Address')); ?>

				<?php echo $this->Form->error('Club.email', null, array('class' => 'error')); ?>
				 <?php //echo $this->Form->password('Club.password', array('maxlength'=>255, 'class'=>'validate[required]','placeholder'=>'Password')); ?>

				<?php //echo $this->Form->error('Club.password', null, array('class' => 'error')); ?>
			
			<?php echo $this->Form->text('Club.club_name', array('maxlength'=>255,'id'=>'ClubName','placeholder'=>'Club Name')); ?>
...
<?php echo $this->Form->text('Club.first_name', array('maxlength'=>255,'id'=>'FirstName','class'=>'validate[required]','placeholder'=>'First name')); ?>
              ...
              <div class="six columns">
				...
				<?php echo $this->Form->text('Club.last_name', array('maxlength'=>255,'id'=>'LastName','class'=>'validate[required]','placeholder'=>'Last name')); ?>
            	...
            	<?php echo $this->Form->text('Club.address', array('maxlength'=>255,'id'=>'Address','placeholder'=>'Address')); ?>

				<?php echo $this->Form->error('Club.address', null, array('class' => 'error')); ?>
           
            <div class="row">
              <div class="six columns">
              <?php echo $this->Form->text('Club.city', array('maxlength'=>255, 'id'=>'city','placeholder'=>'City')); ?>
				
				<?php echo $this->Form->error('Club.city', null, array('class' => 'error')); ?>
               
              </div>
              <div class="six columns">
              <?php echo $this->Form->text('Club.state', array('maxlength'=>255, 'id'=>'state','placeholder'=>'State')); ?>
            ...
            <div class="row">
              <div class="twelve form-select columns">
				...
				<?php echo $this->Form->select('Club.country', $countries, array('style'=>'','onChange'=>'document.getElementById(\'customSelect\').value= this.options[this.selectedIndex].text','default'=>226));?>
				
				<?php echo $this->Form->error('Club.country', null, array('class' => 'error')); ?>
              
               
                <input type="text" id="customSelect" value="<?php if($this->data['Club']['country']!=''){
                foreach($countries as $key=>$val)
                {
                  if($key==$this->data['Club']['country'])
                  {
                  	 echo $val;
                  }	
            ...
            <?php echo $this->Form->text('Club.zip', array('maxlength'=>255, 'id'=>'zip','placeholder'=>'Zip code')); ?>
				
				<?php echo $this->Form->error('Club.zip', null, array('class' => 'error')); ?>
            
            <div class="row">
              <div class="six columns">
              	<?php echo $this->Form->text('Club.phone', array('maxlength'=>255,'id'=>'Phone','placeholder'=>'Phone 555-555-5555','class'=>'validate[required,custom[phone]] text-input','pattern'=>'\d{3}-?\d{3}-?\d{4}')); ?>

				<?php echo $this->Form->error('Club.phone', null, array('class' => 'error')); ?>
              </div>
              <div class="six columns">
                <?php echo $this->Form->text('Club.mobile', array('maxlength'=>255,'id'=>'Mobile','placeholder'=>'Mobile 555-555-5555','class'=>'validate[required,custom[phone]] text-input','pattern'=>'\d{3}-?\d{3}-?\d{4}')); ?>

				<?php echo $this->Form->error('Club.mobile', null, array('class' => 'error')); ?>
              </div>
            </div>
           
            <div class="row">
              <div class="six columns"><span class="file-wrapper">
              	<?php echo $this->Form->file('Club.website_logo');?>
								<?php echo $this->Form->error('Club.website_logo', null, array('class' => 'error'));?>
							</div>
							<div class="fix"></div>
							<?php echo $this->Form->hidden('Club.id'); ?>
							
							 <?php echo $this->Form->hidden('Club.old_image',array('value'=>$this->request->data["Club"]["website_logo"]));?>
							
						</div>
							<div id="imgCont" class="rowElem noborder">
							<p style="margin:0 0 0 0;"><i>Please upload the pic with in 2MB in size</i></p>
						<div class="formRight">
						
							<div style="float:left;<?php if( array_key_exists("website_logo",$this->request->data["Club"]) && !empty($this->request->data["Club"]["website_logo"]) ) { ?>border:1px solid #d8d8d8;<?php } ?>padding:8px;" id="video_container">
							<?php if( array_key_exists("website_logo",$this->request->data["Club"]) && !empty($this->request->data["Club"]["website_logo"]) ) { ?>
								<img border="1"  width="100px" src="<?php echo $config["imgurl"];?>uploads/<?php echo $this->data["Club"]["website_logo"];?>"/>
                ...
                <?php echo $this->Form->textarea('Club.about_us', array('maxlength'=>500,'placeholder'=>'About Us')); ?>

				<?php echo $this->Form->error('Club.about_us', null, array('class' => 'error')); ?>
				</div>
               </div>
                  ...
                  <div id="thirtydays" class="register-form-popup common-overlaycontent"> <a class="close-nav" onclick="popupClose('popchangepass');" id="popchangepass" href="javascript:void(0);"></a>
                    <div class="row register-popup-form">
                      <div class="twelve field-pad columns">
                        ...
                        <form action="/clubs/changepassclub/" controller="club" enctype="multipart/form-data" class="resform-wrap" id="changepassword" method="post" accept-charset="utf-8" onsubmit="return validateconfirmpass();">
						  ...
						  <input type="hidden" id="originalpassword" value="<?php echo $pas = $this->data[Club][password]; ?>" name="originalpassword">
