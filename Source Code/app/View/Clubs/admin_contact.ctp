...
<?php echo $this->Form->create(null,array('controller'=>'clubs', 'action'=>'contact', 'enctype'=>'multipart/form-data', 'class'=>'mainForm', 'id'=>'valid')); ?>
...
<div class="table">
    <div class="head">
		<h5 class="iFrames">Manage Clubs Contacts</h5>
		<div class="rowElem noborder" style="clear:none;margin-top: -4px;">
			<?php if(isset($contacts) && count($contacts) > 0){ ?>
				<?php  echo $this->Form->select('ClubContact.statusTop',unserialize($config['status_array']),array('empty'=>'Select','class'=>'topAction','style'=>'width:20%')); ?>&nbsp;
				<?php  echo $this->Form->submit('OK', array('style'=>'float:none;margin-left: -1px;','class'=>'blueBtn submitForm','name'=>'data[ClubContact][submit]', 'div'=>false, 'onclick'=>"return del('data[ClubContact][id][]')",'id'=>'StatusTop'));?>				
				
			<?php 	} ?> 		
				<a href="<?php echo $this->Html->url(array('controller'=>'clubs', 'action'=>'addcontact')); ?>" style="float: right; margin-top:0px; padding: 2px 13px;margin-right:0px;" class='blueBtn'>Add New</a>
		</div>		
	</div>
        <div class="dataTables_wrapper" id="example_wrapper">
			<?php echo $this->Form->create('ClubContact',array('controller'=>'clubs','action'=>'contact/'.$this->params["pass"][0],'class'=>'mainForm')); ?>	
				<div class="">
			...
			<?php 
				...
				if(isset($clubs) && count($clubs) > 0){
					$this->Paginator->options(array('url' => array("controller"=>"clubs","action"=>"contact/$tab/keyword:$keyword")));
			...
			<table cellspacing="0" cellpadding="0" border="0" id="" class="display">
                <thead>
                    <tr>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
							<div style="padding-right:8px;"><?php echo $this->Form->text('ClubContact.all', array('type'=>'checkbox', 'id'=>'data[ClubContact][all]', 'onclick'=>"selectDeselect('data[ClubContact][id][]', this.name);"));?></div>
						</th>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
				...
				if(isset($contacts) && count($contacts) > 0){
					...
					foreach($contacts as $prow){
						...
						$full='';
					/*if(trim($prow['ClubContact']['full_name']))
						$full = "(".trim($prow['ClubContact']['full_name']).")";
					else
						$full = "";	*/
				?>
				...
				<tr class="gradeA <?php if($count%2==0) echo 'odd'; else echo 'even';?>">
					<td align="center"><?php echo $this->Form->text('ClubContact.id][', array('type'=>'checkbox', 'value'=>$prow['ClubContact']['id'],  'onclick'=>"checkSelection('data[ClubContact][id][]', 'data[ClubContact][all]')"));?></td>
					<td align="center"><?php echo $index;?></td>
					<td><a href="<?php echo $config['url']; ?>admin/clubs/viewcontact/<?php echo $prow['ClubContact']['id']; ?>" target="_blank" ><?php echo $prow['ClubContact']['title']."<br>".$full; ?></a></td>
					<td><?php echo $prow['ClubContact']['email'] ?></td>		
										
					<td align="center"><?php echo date($config['date_format'], strtotime($prow['ClubContact']['added_date'])); ?></td>
					<td align="center"><?php 
						if($prow['ClubContact']['status']==1){
					...
					<td align="center"><?php echo $this->Html->link($this->Html->image('edit-icon.png'),array('controller'=>'clubs','action'=>'editcontact/'.$prow['ClubContact']['id'] ), array('title'=>'Edit contact','escape'=>false));?>
					</td>
