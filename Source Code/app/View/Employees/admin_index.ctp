...
##  Description :		Admin Employee Index
## *****************************************************************
...
//die('here');
 ?>
<?php echo $this->Form->create('Employee' ,array('controller'=>'employees', 'action'=>'index', 'enctype'=>'multipart/form-data', 'class'=>'mainForm', 'id'=>'valid')); ?>
...
<div class="table">
    <div class="head">
		<h5 class="iFrames">Manage Employees</h5>
		<div class="rowElem noborder" style="clear:none;margin-top: -4px;">
			<?php if(isset($employees) && count($employees) > 0){ ?>
				<?php  echo $this->Form->select('Employee.statusTop',unserialize($config['status_array']),array('empty'=>'Select','class'=>'topAction','style'=>'width:20%')); ?>&nbsp;
				<?php  echo $this->Form->submit('OK', array('style'=>'float:none;margin-left: -1px;','class'=>'blueBtn submitForm','name'=>'data[Employee][submit]', 'div'=>false, 'onclick'=>"return del('data[Employee][id][]')",'id'=>'StatusTop'));?>				
				
			<?php 	} ?> 		
				<a href="<?php echo $this->Html->url(array('controller'=>'employees', 'action'=>'add')); ?>" style="float: right; margin-top:0px; padding: 2px 13px;margin-right:0px;" class='blueBtn'>Add New</a>
		</div>		
	</div>
        <div class="dataTables_wrapper" id="example_wrapper">
			<?php echo $this->Form->create('Employee',array('controller'=>'employees','action'=>'index/'.$this->params["pass"][0],'class'=>'mainForm')); ?>	
				<div class="">
					<div class="dataTables_filter" id="example_filter" style="right:15%;top:-38px;">
						<label>Search: <input type="text" name="keyword" value="<?php echo $keyword;?>" placeholder="Search by email or Employee Name..."/>
			...
			<?php 
				...
				if(isset($employees) && count($employees) > 0){
					$this->Paginator->options(array('url' => array("controller"=>"employees","action"=>"index/$tab/keyword:$keyword")));
			...
			<table cellspacing="0" cellpadding="0" border="0" id="" class="display">
                <thead>
                    <tr>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
							<div style="padding-right:8px;"><?php echo $this->Form->text('Employee.all', array('type'=>'checkbox', 'id'=>'data[Employee][all]', 'onclick'=>"selectDeselect('data[Employee][id][]', this.name);"));?></div>
						</th>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
				...
				if(isset($employees) && count($employees) > 0){
					$count = 0; 
					foreach($employees as $prow){
						$index = ((($page-1)*$limit) + ($count+1));
						
					if(trim($prow['Employee']['full_name']))
						$full = trim($prow['Employee']['full_name']);
					else
						$full = "";	
				?>
				...
				<tr class="gradeA <?php if($count%2==0) echo 'odd'; else echo 'even';?>">
					<td align="center"><?php echo $this->Form->text('Employee.id][', array('type'=>'checkbox', 'value'=>$prow['Employee']['id'],  'onclick'=>"checkSelection('data[Employee][id][]', 'data[Employee][all]')"));?></td>
					<td align="center"><?php echo $index;?></td>
					<td><a href="<?php echo $config['url']; ?>admin/Employees/view/<?php echo $prow['Employee']['id']; ?>" target="_blank" ><?php echo $full; ?></a></td>
					<td><?php echo $prow['Employee']['email'] ?></td>		
										
					<td align="center"><?php echo date($config['date_format'], strtotime($prow['Employee']['created_date'])); ?></td>
					<td align="center"><?php 
						if($prow['Employee']['status']==1){
					...
					<td align="center"><?php echo $this->Html->link($this->Html->image('edit-icon.png'),array('controller'=>'employees','action'=>'edit/'.$prow['Employee']['id'] ), array('title'=>'Edit Employee','escape'=>false));?>
					</td>
				...
				<?php if(isset($employees) && count($employees) > 0){?>
					...
					<div class="rowElem noborder">
						<?php  //echo $this->Form->select('Employee.status',unserialize($config['status_array']),array('empty'=>'Select', 'id'=>'Status','style'=>'width:20%;float:left;')); ?>&nbsp;
						<?php  //echo $this->Form->submit('OK', array('style'=>'float:left;','class'=>'blueBtn submitForm','name'=>'data[Employee][submit]', 'div'=>false, 'onclick'=>"return del('data[Employee][id][]')"));?>
					</div>
					</td></tr>
				<?php }?>
				</tbody>
