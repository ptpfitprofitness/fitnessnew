...
<?php echo $this->Form->create(null,array('controller'=>'helpguides', 'action'=>'helpguide',  'class'=>'mainForm', 'id'=>'valid')); ?>
...
<div class="table">
    <div class="head">
		<h5 class="iFrames">Manage Help Guides</h5>
		<div class="rowElem noborder" style="clear:none;margin-top: -4px;">
			<?php if(isset($branches) && count($branches) > 0){ ?>
				<?php  echo $this->Form->select('HelpGuide.statusTop',unserialize($config['status_array']),array('empty'=>'Select','class'=>'topAction','style'=>'width:20%')); ?>&nbsp;
				<?php  echo $this->Form->submit('OK', array('style'=>'float:none;margin-left: -1px;','class'=>'blueBtn submitForm','name'=>'data[HelpGuide][submit]', 'div'=>false, 'onclick'=>"return del('data[HelpGuide][id][]')",'id'=>'StatusTop'));?>				
				
			<?php 	} ?> 		
				<a href="<?php echo $this->Html->url(array('controller'=>'helpguides', 'action'=>'addhelpguide')); ?>" style="float: right; margin-top:0px; padding: 2px 13px;margin-right:0px;" class='blueBtn'>Add New</a>
		</div>		
	</div>
        <div class="dataTables_wrapper" id="example_wrapper">
			<?php echo $this->Form->create('HelpGuide',array('controller'=>'helpguides','action'=>'helpguide/'.$this->params["pass"][0],'class'=>'mainForm')); ?>	
				<div class="">
			...
			<?php 
				...
				if(isset($branches) && count($branches) > 0){
					$this->Paginator->options(array('url' => array("controller"=>"helpguides","action"=>"helpguide/$tab/keyword:$keyword")));
			...
			<table cellspacing="0" cellpadding="0" border="0" id="" class="display">
                <thead>
                    <tr>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
							<div style="padding-right:8px;"><?php echo $this->Form->text('HelpGuide.all', array('type'=>'checkbox', 'id'=>'data[HelpGuide][all]', 'onclick'=>"selectDeselect('data[HelpGuide][id][]', this.name);"));?></div>
						</th>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
				...
				if(isset($branches) && count($branches) > 0){
					...
					if(trim($prow['HelpGuide']['doc_name']))
						$full = "(".trim($prow['HelpGuide']['doc_name']).")";
					else
						$full = "";	
				?>
				...
				<tr class="gradeA <?php if($count%2==0) echo 'odd'; else echo 'even';?>">
					<td align="center"><?php echo $this->Form->text('HelpGuide.id][', array('type'=>'checkbox', 'value'=>$prow['HelpGuide']['id'],  'onclick'=>"checkSelection('data[HelpGuide][id][]', 'data[HelpGuide][all]')"));?></td>
					<td align="center"><?php echo $index;?></td>
					<td><?php echo $prow['HelpGuide']['doc_name']; ?><br/>
					<a href="javascript:void(0);" onclick="viewvideo('<?php echo base64_encode($prow['HelpGuide']['id']); ?>')">View Video</a>
					</td>
					<td><?php echo $prow['HelpGuide']['description'] ?></td><td><?php echo $prow['HelpGuide']['user_type'] ?></td>		
										
					<td align="center"><?php echo date($config['date_format'], strtotime($prow['HelpGuide']['added_date'])); ?></td>
					<td align="center"><?php 
						if($prow['HelpGuide']['status']==1){
					...
					<!--<td align="center"><?php echo $this->Html->link($this->Html->image('edit-icon.png'),array('controller'=>'helpguides','action'=>'edithelpguide/'.$prow['HelpGuide']['id'] ), array('title'=>'Edit Help Guide','escape'=>false));?>
					</td>-->
