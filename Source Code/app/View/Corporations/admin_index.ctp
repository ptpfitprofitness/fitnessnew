...
<?php echo $this->Form->create('Corporation' ,array('controller'=>'corporations', 'action'=>'index',  'class'=>'mainForm', 'id'=>'valid')); ?>
...
<div class="table">
    <div class="head">
		<h5 class="iFrames">Manage Corporations</h5>
		<div class="rowElem noborder" style="clear:none;margin-top: -4px;">
			<?php if(isset($corporations) && count($corporations) > 0){ ?>
				<?php  echo $this->Form->select('Corporation.statusTop',unserialize($config['status_array']),array('empty'=>'Select','class'=>'topAction','style'=>'width:20%')); ?>&nbsp;
				<?php  echo $this->Form->submit('OK', array('style'=>'float:none;margin-left: -1px;','class'=>'blueBtn submitForm','name'=>'data[Corporation][submit]', 'div'=>false, 'onclick'=>"return del('data[Corporation][id][]')",'id'=>'StatusTop'));?>				
				
			<?php 	} ?> 		
				<a href="<?php echo $this->Html->url(array('controller'=>'corporations', 'action'=>'add')); ?>" style="float: right; margin-top:0px; padding: 2px 13px;margin-right:0px;" class='blueBtn'>Add New</a>
		</div>		
	</div>
        <div class="dataTables_wrapper" id="example_wrapper">
			<?php echo $this->Form->create('Corporation',array('controller'=>'corporations','action'=>'index/'.$this->params["pass"][0],'class'=>'mainForm')); ?>	
				<div class="">
					<div class="dataTables_filter" id="example_filter" style="right:15%;top:-38px;">
						<label>Search: <input type="text" name="keyword" value="<?php echo $keyword;?>" placeholder="Search by email or Corporation Name..."/>
			...
			<?php 
				...
				if(isset($corporations) && count($corporations) > 0){
					$this->Paginator->options(array('url' => array("controller"=>"corporations","action"=>"index/$tab/keyword:$keyword")));
			...
			<table cellspacing="0" cellpadding="0" border="0" id="" class="display">
                <thead>
                    <tr>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
							<div style="padding-right:8px;"><?php echo $this->Form->text('Corporation.all', array('type'=>'checkbox', 'id'=>'data[Corporation][all]', 'onclick'=>"selectDeselect('data[Corporation][id][]', this.name);"));?></div>
						</th>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
				...
				if(isset($corporations) && count($corporations) > 0){
					$count = 0; 
					foreach($corporations as $prow){
						$index = ((($page-1)*$limit) + ($count+1));
						
					if(trim($prow['Corporation']['company_name']))
						$full = trim($prow['Corporation']['company_name']);
					else
						$full = "";	
				?>
				...
				<tr class="gradeA <?php if($count%2==0) echo 'odd'; else echo 'even';?>">
					<td align="center"><?php echo $this->Form->text('Corporation.id][', array('type'=>'checkbox', 'value'=>$prow['Corporation']['id'],  'onclick'=>"checkSelection('data[Corporation][id][]', 'data[Corporation][all]')"));?></td>
					<td align="center"><?php echo $index;?></td>
					<td><a href="<?php echo $config['url']; ?>admin/corporations/view/<?php echo $prow['Corporation']['id']; ?>" target="_blank" ><?php echo $full; ?></a></td>
					<td><?php echo $prow['Corporation']['email'] ?></td>		
										
					<td align="center"><?php echo date($config['date_format'], strtotime($prow['Corporation']['added_date'])); ?></td>
					<td align="center"><?php 
						if($prow['Corporation']['status']==1){
					...
					<td align="center"><?php echo $this->Html->link($this->Html->image('edit-icon.png'),array('controller'=>'corporations','action'=>'edit/'.$prow['Corporation']['id'] ), array('title'=>'Edit Corporation','escape'=>false));?>
					</td>
