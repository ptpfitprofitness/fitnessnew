...
<?php echo $this->Form->create(null ,array('controller'=>'corporations', 'action'=>'branch', 'enctype'=>'multipart/form-data', 'class'=>'mainForm', 'id'=>'valid')); ?>
...
<div class="table">
    <div class="head">
		<h5 class="iFrames">Manage corporations Branches</h5>
		<div class="rowElem noborder" style="clear:none;margin-top: -4px;">
			<?php if(isset($branches) && count($branches) > 0){ ?>
				<?php  echo $this->Form->select('CorporationBranch.statusTop',unserialize($config['status_array']),array('empty'=>'Select','class'=>'topAction','style'=>'width:20%')); ?>&nbsp;
				<?php  echo $this->Form->submit('OK', array('style'=>'float:none;margin-left: -1px;','class'=>'blueBtn submitForm','name'=>'data[CorporationBranch][submit]', 'div'=>false, 'onclick'=>"return del('data[CorporationBranch][id][]')",'id'=>'StatusTop'));?>				
				
			<?php 	} ?> 		
				<a href="<?php echo $this->Html->url(array('controller'=>'corporations', 'action'=>'addbranch')); ?>" style="float: right; margin-top:0px; padding: 2px 13px;margin-right:0px;" class='blueBtn'>Add New</a>
		</div>		
	</div>
        <div class="dataTables_wrapper" id="example_wrapper">
			<?php echo $this->Form->create('CorporationBranch',array('controller'=>'corporations','action'=>'branch/','class'=>'mainForm')); ?>	
				<div class="">
			...
			<?php 
				...
				if(isset($corporations) && count($corporations) > 0){
					$this->Paginator->options(array('url' => array("controller"=>"corporations","action"=>"branch/$tab/keyword:$keyword")));
			...
			<table cellspacing="0" cellpadding="0" border="0" id="" class="display">
                <thead>
                    <tr>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
							<div style="padding-right:8px;"><?php echo $this->Form->text('CorporationBranch.all', array('type'=>'checkbox', 'id'=>'data[CorporationBranch][all]', 'onclick'=>"selectDeselect('data[CorporationBranch][id][]', this.name);"));?></div>
						</th>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
				...
				if(isset($branches) && count($branches) > 0){
					...
					if(trim($prow['CorporationBranch']['full_name']))
						$full = "(".trim($prow['CorporationBranch']['full_name']).")";
					else
						$full = "";	
				?>
				...
				<tr class="gradeA <?php if($count%2==0) echo 'odd'; else echo 'even';?>">
					<td align="center"><?php echo $this->Form->text('CorporationBranch.id][', array('type'=>'checkbox', 'value'=>$prow['CorporationBranch']['id'],  'onclick'=>"checkSelection('data[CorporationBranch][id][]', 'data[CorporationBranch][all]')"));?></td>
					<td align="center"><?php echo $index;?></td>
					<td><a href="<?php echo $config['url']; ?>admin/corporations/viewbranch/<?php echo $prow['CorporationBranch']['id']; ?>" target="_blank" ><?php echo $prow['CorporationBranch']['branch_name']."<br>".$full; ?></a></td>
					<td><?php echo $prow['CorporationBranch']['email'] ?></td>		
										
					<td align="center"><?php echo date($config['date_format'], strtotime($prow['CorporationBranch']['added_date'])); ?></td>
					<td align="center"><?php 
						if($prow['CorporationBranch']['status']==1){
					...
					<td align="center"><?php echo $this->Html->link($this->Html->image('edit-icon.png'),array('controller'=>'corporations','action'=>'editbranch/'.$prow['CorporationBranch']['id'] ), array('title'=>'Edit Branch','escape'=>false));?>
					</td>
