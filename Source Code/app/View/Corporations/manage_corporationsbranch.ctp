...
function edittrainer(str)
{
   ...
   if(str!='')
   {
   	document.location.href="<?php echo $config['url'];?>corporations/editcorporation_branch/"+str;
...
function newtrainee()
{
	...
	document.location.href="<?php echo $config['url'];?>corporations/addcorporation_branch/";
}
...
function deletetrainer(str)
{
	if(str!='')
	{
		if(confirm("Are you sure, you want to delete this Corporation Branch?"))
		{
	         	$.ajax({
				url:"<?php echo $config['url'];?>corporations/deletecorporation_branch/",
				type:"POST",
				data:{id:str},
				success:function(e) {
					var response = eval(' ( '+e+' ) ');
     ...
     <?php echo $this->element('leftcorporation');?>
      <div class="eight inside-head columns">
        <ul class="profile-tabs-list desktop-tabs clearfix">
          <li><a href="#Profile" class="active"><span class="profile-ico9"><img src="<?php echo $config['url'];?>images/client_infoico.png"></span>My Corporation Branch</a></li>
        
        </ul>    
        <div style="float:right">
        <input type="button" class="change-pic-nav" onclick="newtrainee();" value="Add New Corporation Branch" name="submit">
        </div>
        <div class="main-responsive-box"><ul class="listing-box all-headtabs">
          <li class="listing-heading">
            ...
            <div class="list-colum second-tabs">Corporation Branch Name</div>
         ...
         //pr($clients);
           foreach($corporations as $corporation)
            ...
            <li>
              ...
              <div class="list-colum second-tabs"><?php echo $corporation['CorporationBranch']['branch_name'];?></div>
              <div class="list-colum third-tabs"><?php echo $corporation['CorporationBranch']['email'];?></div>
              <div class="list-colum fifth-tabs"><a href="javascript:void(0);" onclick="edittrainer('<?php echo base64_encode($corporation['CorporationBranch']['id']);?>');"><img src="<?php echo $config['url'];?>images/editicon.png"></a>  <a href="javascript:void(0);" onclick="deletetrainer(<?php echo $corporation['CorporationBranch']['id'];?>);"><img src="<?php echo $config['url'];?>images/deleteicon.png"></a></div>
            </li>
         <?php   $cnt++; ?>
            <?php  }?>
                      ...
                      <div class="twelve field-pad columns">
                        <form action="/fitnessAaland/corporations/uploadpic/" controller="home" enctype="multipart/form-data" class="resform-wrap" id="valid" method="post" accept-charset="utf-8" onsubmit="return validuppic();">
                          <h2>Upload Profile Pic</h2>
                           <input type="file" name="data['Corporation']['logo']" id="TrainerLogo" />
