...
<?php echo $this->Form->create(null,array('controller'=>'corporations', 'action'=>'contact', 'enctype'=>'multipart/form-data', 'class'=>'mainForm', 'id'=>'valid')); ?>
...
<div class="table">
    <div class="head">
		<h5 class="iFrames">Manage corporations Contacts</h5>
		<div class="rowElem noborder" style="clear:none;margin-top: -4px;">
			<?php if(isset($contacts) && count($contacts) > 0){ ?>
				<?php  echo $this->Form->select('CorporationContact.statusTop',unserialize($config['status_array']),array('empty'=>'Select','class'=>'topAction','style'=>'width:20%')); ?>&nbsp;
				<?php  echo $this->Form->submit('OK', array('style'=>'float:none;margin-left: -1px;','class'=>'blueBtn submitForm','name'=>'data[CorporationContact][submit]', 'div'=>false, 'onclick'=>"return del('data[CorporationContact][id][]')",'id'=>'StatusTop'));?>				
				
			<?php 	} ?> 		
				<a href="<?php echo $this->Html->url(array('controller'=>'corporations', 'action'=>'addcontact')); ?>" style="float: right; margin-top:0px; padding: 2px 13px;margin-right:0px;" class='blueBtn'>Add New</a>
		</div>		
	</div>
        <div class="dataTables_wrapper" id="example_wrapper">
			<?php echo $this->Form->create('CorporationContact',array('controller'=>'corporations','action'=>'contact/'.$this->params["pass"][0],'class'=>'mainForm')); ?>	
				<div class="">
					<div class="dataTables_filter" id="example_filter" style="right:15%;top:-38px;">
						<label>Search: <input type="text" name="keyword" value="<?php echo $keyword;?>" placeholder="Search by email or contact Name..."/>
			...
			<?php 
				...
				if(isset($corporations) && count($corporations) > 0){
					$this->Paginator->options(array('url' => array("controller"=>"corporations","action"=>"contact/$tab/keyword:$keyword")));
			...
			<table cellspacing="0" cellpadding="0" border="0" id="" class="display">
                <thead>
                    <tr>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
							<div style="padding-right:8px;"><?php echo $this->Form->text('CorporationContact.all', array('type'=>'checkbox', 'id'=>'data[CorporationContact][all]', 'onclick'=>"selectDeselect('data[CorporationContact][id][]', this.name);"));?></div>
						</th>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
				...
				if(isset($contacts) && count($contacts) > 0){
					$count = 0; 
					foreach($contacts as $prow){
						$index = ((($page-1)*$limit) + ($count+1));
						$full='';
					/*if(trim($prow['CorporationContact']['full_name']))
						$full = "(".trim($prow['CorporationContact']['full_name']).")";
					else
						$full = "";	*/
				?>
				...
				<tr class="gradeA <?php if($count%2==0) echo 'odd'; else echo 'even';?>">
					<td align="center"><?php echo $this->Form->text('CorporationContact.id][', array('type'=>'checkbox', 'value'=>$prow['CorporationContact']['id'],  'onclick'=>"checkSelection('data[CorporationContact][id][]', 'data[CorporationContact][all]')"));?></td>
					<td align="center"><?php echo $index;?></td>
					<td><a href="<?php echo $config['url']; ?>admin/corporations/viewcontact/<?php echo $prow['CorporationContact']['id']; ?>" target="_blank" ><?php echo $prow['CorporationContact']['title']."<br>".$full; ?></a></td>
					<td><?php echo $prow['CorporationContact']['email'] ?></td>		
										
					<td align="center"><?php echo date($config['date_format'], strtotime($prow['CorporationContact']['added_date'])); ?></td>
					<td align="center"><?php 
						if($prow['CorporationContact']['status']==1){
					...
					<td align="center"><?php echo $this->Html->link($this->Html->image('edit-icon.png'),array('controller'=>'corporations','action'=>'editcontact/'.$prow['CorporationContact']['id'] ), array('title'=>'Edit contact','escape'=>false));?>
					</td>
