...
<section class="contentContainer clearfix">
    ...
    <div class="row">
      ...
      <div class="eight inside-head columns">
        ...
        <h2> Edit Employee</h2>
      <form accept-charset="utf-8" method="post" id="validmyempt" class="resform-wrap" enctype="multipart/form-data" controller="corporations" action="/fitnessAaland/corporations/edit_employee/<?php echo base64_decode($cid);?>" >
       ...
       <input type="hidden"  name="data[Employee][id]" id="EmployeeId" value="<?php echo $cid;?>"/>
          ...
          foreach($branches as $key=>$val)
            {
            	if($this->data['Employee']['branch_id']==$key)
            	{
            		 $vbrch=$val;
            	}
            	?>
            	 <option value='<?php echo $key; ?>' <?php if($this->data['Employee']['branch_id']==$key){echo 'selected="selected"';}?>><?php echo $val;?></option>
            	<?php
            	 }
           ?> 	
            </select>
            <input type="text" id="customSelectGval" value="<?php echo $vbrch;?>"/>
          </div>
          ...
          <?php //echo $this->Form->text('Employee.username', array('maxlength'=>255,'id'=>'Username','placeholder'=>'Username','class'=>'validate[required] ')); ?>
	        ...
	        <div class="row">
	          ...
	          <div class="six columns form-select">
				...
				<?php 
          $vcntry='-- Select Country --';
          foreach($countries as $key=>$val)
            {
            	if($this->data['Employee']['country']==$key)
            	{
            		 $vcntry=$val;
            	}
            }
				...
				<input type="text" value="<?php echo $vcntry;?>" id="customSelectCovval"/>
	          </div>
	          <div class="six columns">
 ...
 ?>
          ...
          foreach($notify as $key=>$val)
            {
            	if($this->data['Employee']['notification_status']==$key)
            	{
            		 $vcntv=$val;
            	}
            }
            	?>
				<input type="text" value="<?php echo $vcntv;?>" id="customSelectNotval"/>
	          </div>
	          <div class="six columns">
