...
function edittrainer(str)
{
   ...
   if(str!='')
   {
   	document.location.href="<?php echo $config['url'];?>corporations/editmy_contact/"+str;
...
function deletetrainer(str)
{
	if(str!='')
	{
		...
		{
	         	$.ajax({
				url:"<?php echo $config['url'];?>corporations/deletecontact/",
				type:"POST",
				data:{id:str},
				success:function(e) {
					var response = eval(' ( '+e+' ) ');
     ...
     <?php echo $this->element('leftcorporation');?>
      <div class="eight inside-head columns">
        <ul class="profile-tabs-list desktop-tabs clearfix">
          <li><a href="#Profile" class="active"><span class="profile-ico9"><img src="<?php echo $config['url'];?>images/client_infoico.png"></span>My Contacts</a></li>
        
        </ul>    
        <div style="float:right">
        <input type="button" style="width:120px;" class="change-pic-nav" onclick="newtrainee();" value="Add New Contact" name="submit">
        </div>
         ...
         //pr($clients);
           foreach($setSvftArr as $contact)
            ...
            <li>
              ...
              <div class="list-colum second-tabs"><?php echo $contact['CorporationContact']['title'];?></div>
              <div class="list-colum third-tabs"><?php echo $contact['CorporationContact']['email'];?></div>
              <div class="list-colum fifth-tabs"><a href="javascript:void(0);" onclick="edittrainer('<?php echo base64_encode($contact['CorporationContact']['id']);?>');"><img src="<?php echo $config['url'];?>images/editicon.png"></a>  <a href="javascript:void(0);" onclick="deletetrainer(<?php echo $contact['CorporationContact']['id'];?>);"><img src="<?php echo $config['url'];?>images/deleteicon.png"></a></div>
            </li>
         <?php   $cnt++; ?>
            <?php  }?>
