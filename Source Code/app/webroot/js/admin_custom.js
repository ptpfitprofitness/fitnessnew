...
{	
	var foo = []; 
	$('#BranchClubId :selected').each(function(i, selected){ 
	  foo[i] = $(selected).val(); 
	});	
	if(foo.length > 0){			
			$.ajax({
				type:"POST",
				url: site_url + 'clubs/listbranches',
				data: "club_branches="+foo,
				success:function(data){						
					$("#branchesID").html(data);					
				}
			});
		}	 
}
function getCorporationBranchList()
{	
	var foo = []; 
	$('#BranchCorporationId :selected').each(function(i, selected){ 
	  foo[i] = $(selected).val(); 
	});	
	if(foo.length > 0){			
			$.ajax({
				type:"POST",
				url: site_url + 'corporations/listbranches',
				data: "corporation_branches="+foo,
				success:function(data){						
					$("#branchesID").html(data);					
				}
			});
...
function getCorporationBranchListEmp()
{	
	var foo = []; 
	$('#BranchCorporationId :selected').each(function(i, selected){ 
	  foo[i] = $(selected).val(); 
	...
	if(foo.length > 0){			
			$.ajax({
				type:"POST",
				url: site_url + 'employees/listbranches',
				data: "corporation_branches="+foo,
				success:function(data){						
					$("#branchesID").html(data);					
				}
