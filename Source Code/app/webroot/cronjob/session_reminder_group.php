...
$sql = "SELECT  trainees.email as trainee_email,gm.client_id,gm.client_name,schedule_calendar.id,schedule_calendar.appointment_type,schedule_calendar.session_type,schedule_calendar.trainer_id,schedule_calendar.trainee_id,schedule_calendar.start,schedule_calendar.added_date,schedule_calendar.posted_by,trainees.first_name as trainee_fname,trainees.session_reminder_notification as trainee_session_reminder_notification,trainers.email as trainers_email,trainers.first_name as trainers_fname,trainers.phone as trainers_phone,trainers.website_logo as trainers_website_logo
FROM schedule_calendar 
inner JOIN group_members gm ON schedule_calendar.trainee_id=gm.group_id
INNER JOIN trainees ON gm.client_id=trainees.id
INNER JOIN trainers ON schedule_calendar.trainer_id=trainers.id";
...
if (mysql_num_rows($result) > 0) {
	...
	$time_formated = date('h:i:s A',strtotime($row["start"]));
	if ($date_diff <= 1 && $date_diff > 0 && $row["appointment_type"] == "Booked" && $row["trainee_session_reminder_notification"] == 1 &&  $row["posted_by"] == 'Group')
		{
			...
			$message .= '<img style="width:100px;" src="http://www.ptpfitpro.com/uploads/'.$row["trainers_website_logo"].'"/>';
