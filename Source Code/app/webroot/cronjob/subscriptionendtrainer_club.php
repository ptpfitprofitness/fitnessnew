...
if (mysql_num_rows($payment_result) > 0) {
	...
	else if ($date_diff ==-1 && ($payment_row["subscriptionplan"] != '' || $payment_row["subscriptionplan"] != NULL))
	{	
		$email_dea = $payment_row["trainer_email"];
		$sql_deactivate = "UPDATE `trainers` SET `status`= '0' WHERE `email`= '$email_dea' AND `trainer_type` = 'I'";
		$result_deactivate = mysql_query($sql_deactivate) or die("Query Error ".mysql_error());
		
		$sql_deactivate_club = "UPDATE `clubs` SET `status`= '0' WHERE `email`= '$email_dea'";
		$result_deactivate_club = mysql_query($sql_deactivate_club) or die("Query Error ".mysql_error());
		//echo "Row effected";
