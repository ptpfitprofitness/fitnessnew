...
##  Description :		This file contains function for Corporation 
## *****************************************************************
 ...
 */
	class CorporationsController extends AppController {
		...
		public $name 		= 'Corporations';
		public $helpers 	= array('Html','Session','Cksource');
		public $uses 		= array('Country','Member','Club','Corporation','CorporationBranch','CorporationContact','Employee','Trainer','Trainee','TraineeClub','TraineeTrainer','Managemail');
		...
		public function admin_add(){
			...
			if(!empty($this->data)) {
				...
				$this->Corporation->set($this->data);
				if($this->Corporation->validates()) {
						
					    
					    $this->request->data["Corporation"]["username"] 		    = $this->data['Corporation']['email'];
					    $this->request->data["Corporation"]["added_date"] 		    = date("Y-m-d h:i:s");
						$this->request->data["Corporation"]["modified_date"] 		    = date("Y-m-d h:i:s");
					    	
						if($this->Corporation->save($this->data)) {				
							$this->Session->setFlash('Corporation has been created successfully.');
							$this->redirect('/admin/corporations/');
						} else {
		...
		public function admin_view(){
			...
			if(!empty($this->params["pass"][0])) {
				$this->set("corporationInfo",$this->Corporation->find("first",array("conditions"=>array("Corporation.id"=>$this->params["pass"][0]))));	
			}else{
		...
		public function admin_edit($id = null)
		{
			...
			$this->Corporation->set($this->data);
			$this->Corporation->id = $this->data['Corporation']['id'];		
			...
			if($this->Corporation->validates()) {
				...
				$this->request->data["Corporation"]["modified_date"] 		    = date("Y-m-d h:i:s");
				if($this->Corporation->save($this->data)) {
					$this->Session->setFlash('Corporation information has been updated successfully.');
					$this->redirect('/admin/corporations/');
				} else {
					$this->Session->setFlash('Some error has been occured. Please try again.');
				}
				...
				if(is_numeric($id) && $id > 0) {
						...
						$this->request->data = $this->Corporation->read();
					} else {
						$this->Session->setFlash('Invalid Corporation id.');
						$this->redirect('/admin/corporations/');
				}
			}	
		}
		...
		{			
			...
			if(!empty($this->data)){	
				...
				if( array_key_exists("keyword",$this->data) && !empty($this->data["keyword"]) && ($this->data["keyword"] != "Search by email or Corporation Name...") ) {					
					$conditions["OR"] = array(
												"Corporation.company_name LIKE" => "%".$this->data["keyword"]."%",
												"Corporation.email LIKE" => "%".$this->data["keyword"]."%"
											);
					if( !empty($this->params["named"]["keyword"]) )						
						$keyword = $this->params["named"]["keyword"];					
					
				}else{						
						if( !empty($this->data['Corporation']['statusTop']) ) {
							
							$action = $this->data['Corporation']['statusTop'];
						}elseif( !empty($this->data['Corporation']['status'])) {
							
							$action = $this->data['Corporation']['status'];
						}
						
						
						if(isset($this->data['Corporation']['id']) && count($this->data['Corporation']['id']) > 0) {
							
							$this->update_status(trim($action), $this->data['Corporation']['id'], count($this->data['Corporation']['id']));
						} else {
							
							
							if(isset($this->data["submit"]) && isset($this->data["keyword"]) && ($this->data["keyword"]=='' || $this->data["keyword"]=='Search by email or Corporation Name...') && $this->data["submit"]=='Search'){
			...
			if( !empty($this->params["named"]["keyword"]) ) {
				$conditions["OR"] = array(
									"Corporation.company_name LIKE" => "%".$this->params["named"]["keyword"]."%",
									"Corporation.email LIKE" => "%".$this->params["named"]["keyword"]."%"
								);
				$keyword = $this->params["named"]["keyword"];
			}			
					
			
			$this->paginate = array("conditions"=>$conditions,'limit' => '10', 'order' => array('Corporation.company_name' => 'ASC'));
			$corporations = $this->paginate('Corporation'); //default take the current
			$this->set('corporations', $corporations);
			...
			$this->set('limit', $this->params['request']['paging']['Corporation']['options']['limit']);
			$this->set('page', $this->params['request']['paging']['Corporation']['options']['page']);
		}
		...
		$this->set("countries",$this->Country->find('list',array('fields'=>array('Country.id','Country.name'))));	
			...
			$country_id=$setSpecalistArr[$dbusertype]['country'];
			$cntry_array=$this->Country->find('first',array('conditions'=>array('Country.id'=>$country_id)));
			

			$con_name=$cntry_array['Country']['name'];
		...
		{
			 $this->layout = "homelayout";	
		    $this->set("leftcheck",'homemy_emps');
			$dbusertype = $this->Session->read('UTYPE');					
			$this->set("dbusertype",$dbusertype);
			...
			if(!empty($this->data)) {
				...
				if($this->Employee->validates()) {
						...
						if($this->Employee->save($this->data['Employee'])) {
							...
							$this->redirect('/corporations/manage_employees/');
						} else {
							$this->Session->setFlash('Some error has been occured. Please try again.');
						}
				}	
		...
		public function edit_employee($cid=null){
			$this->set("countries",$this->Country->find('list',array('fields'=>array('Country.id','Country.name'))));	
			 $this->layout = "homelayout";	
		    $this->set("leftcheck",'homemy_emps');
			$dbusertype = $this->Session->read('UTYPE');					
			...
			if($this->Employee->validates()) {				
				...
				if($this->Employee->save($this->data)) {
					...
					if(isset($this->request->data['Employee']['trainee_flag']) &&($this->request->data['Employee']['trainee_flag']==1)){
								...
								$makeTrainee=array();
								//$this->Trainee->id=
								$makeTrainee['Trainee']['employee_id']=$cid;
								$makeTrainee['Trainee']['corporation_id']=$this->request->data['Employee']['corporation_id'];
								$makeTrainee['Trainee']['corporation_branch_id']=$this->request->data['Employee']['branch_id'];
								$makeTrainee['Trainee']['mobile']=$this->request->data['Employee']['mobile'];
								$makeTrainee['Trainee']['phone']=$this->request->data['Employee']['phone'];
						...
						elseif(isset($this->request->data['Employee']['trainee_flag']) &&($this->request->data['Employee']['trainee_flag']==0)) {
							$this->Trainee->deleteAll(array('Trainee.employee_id' =>$cid),FALSE);
						}
						else {
							return true;
						}
		...
		{
			 $this->layout = "homelayout";	
		    $this->set("leftcheck",'homemy_contacts');
			$dbusertype = $this->Session->read('UTYPE');					
			$this->set("dbusertype",$dbusertype);
			$id = $this->Session->read('USER_ID');
			$setSpecalistArr=$this->$dbusertype->find("first",array("conditions"=>array("$dbusertype.id"=>$id)));
...
//print_r($setSvftArr); die();
		...
		public function addmy_contact(){
			...
			if(!empty($this->data)) {
				...
				if($this->CorporationContact->validates()) {
						...
						if($this->CorporationContact->save($this->data)) {	
										
							$this->Session->setFlash('Contact has been created successfully.');
							$this->redirect('/corporations/manage_contacts/');
						} else {
							$this->Session->setFlash('Some error has been occured. Please try again.');
						}
				}	
		...
		public function editmy_contact($cid = null)
		{
			...
			if(!empty($this->data)) {
		           $this->CorporationContact->set($this->data);
			$this->CorporationContact->id = $this->data['CorporationContact']['id'];
				...
				if($this->CorporationContact->validates()) {
						...
						if($this->CorporationContact->save($this->data)) {	
							...
							$this->Session->setFlash('Contact has been updated successfully.');
							$this->redirect('/corporations/manage_contacts/');
						} else {
							$this->Session->setFlash('Some error has been occured. Please try again.');
						}
			...
			else 
			{
				...
				if(is_numeric($cid) && $cid > 0) {
						$this->CorporationContact->id = $cid;
						$this->request->data = $this->CorporationContact->read();
					} else {
						$this->Session->setFlash('Invalid Corporation Contact id.');
						$this->redirect('/corporations/manage_contacts/');
				}
			}
		...
		public function manage_corporationsbranch()
		{
			$this->layout = "homelayout";	
		    $this->set("leftcheck",'homemy_corporations');
			$dbusertype = $this->Session->read('UTYPE');					
			$this->set("dbusertype",$dbusertype);
			$id = $this->Session->read('USER_ID');
			$setSpecalistArr=$this->$dbusertype->find("first",array("conditions"=>array("$dbusertype.id"=>$id)));
...
$setSvftArr=$this->CorporationBranch->find("all",array("conditions"=>array("CorporationBranch.corporation_id"=>$id), 'order' => array('CorporationBranch.id' => 'DESC')));
			
//print_r($setSvftArr); die();
			
			$this->set("corporations",$setSvftArr);
				
			
		}
		
		public function addcorporation_branch(){
			...
			$this->set("corporation",$this->Corporation->find('list',array('fields'=>array('Corporation.id','Corporation.company_name'))));
			...
			if(!empty($this->data)) {
				...
				if($this->CorporationBranch->validates()) {
					    ...
					    $this->request->data["CorporationBranch"]["username"] 		    =$this->data["CorporationBranch"]["email"];
						...
						$this->request->data["CorporationBranch"]["modified_date"] 		    = date("Y-m-d h:i:s");
					    	//pr($this->request->data);
					    //$this->loadModel('CorporationBranch');
					   
						if($this->CorporationBranch->save($this->request->data)) {	
										
							$this->Session->setFlash('Branch has been created successfully.');
							$this->redirect('/corporations/manage_corporationsbranch/');
						} else {
							$this->Session->setFlash('Some error has been occured. Please try again.');
						}
				}	
		...
		public function editcorporation_branch($bid = null)
		{
			...
			$this->set("corporation",$this->Corporation->find('list',array('fields'=>array('Corporation.id','Corporation.company_name'))));
			$selected=$this->CorporationBranch->find('list',array('conditions'=>array('CorporationBranch.corporation_id'=>$id),'fields'=>array('CorporationBranch.id','CorporationBranch.branch_name')));
			$this->set('selected',$selected);
			if(!empty($this->data)){
			
			$this->CorporationBranch->set($this->data);
			$this->CorporationBranch->id = $this->data['CorporationBranch']['id'];		
			...
			if($this->CorporationBranch->validates()) {
				...
				$this->request->data["CorporationBranch"]["username"]=$this->data["CorporationBranch"]["email"];
				$this->request->data["CorporationBranch"]["modified_date"] 		    = date("Y-m-d h:i:s");
				if($this->CorporationBranch->save($this->data)) {
				...
				if(is_numeric($bid) && $bid > 0) {
						$this->CorporationBranch->id = $bid;
						$this->request->data = $this->CorporationBranch->read();
					} else {
		...
		public function deletecorporation_branch()
		{
			$this->layout = '';
			$this->render = false;
			if(trim($_POST['id'])!='')
		...
		public function editprofile()
		{
		  ...
		  $this->set("setSpecalistArr",$setSpecalistArr);
			...
			if($this->Corporation->validates()) {
				...
				if(!empty($this->request->data["Corporation"]["logo"]["name"]))
				{
					$filename = $this->request->data["Corporation"]["logo"]["name"];
					$target = $this->config["upload_path"];
					$this->Upload->upload($this->data["Corporation"]["logo"], $target, null, null);
  					$this->request->data["Corporation"]["logo"] = $this->Upload->result; 
  					$picPath = $this->config["upload_path"].$this->request->data["Corporation"]["old_image"];
					@unlink($picPath);
				}else{	
					
					if(!empty($this->request->data["Corporation"]["old_image"])){
						$this->request->data["Corporation"]["logo"] = $this->request->data["Corporation"]["old_image"];			
					}
					else{
						$this->request->data["Corporation"]["logo"] = "";
					}
				}
				$this->request->data["Corporation"]["username"] 		    =$this->data["Corporation"]["email"];
				$this->request->data["Corporation"]["modified_date"] 		    = date("Y-m-d h:i:s");
			  ...
			  if(!empty($this->request->data["Corporation"]["logo"]["name"]))
				{
					...
					@unlink($picPath);
					$this->request->data["Corporation"]["modified_date"] 		    = date("Y-m-d h:i:s");
					$this->request->data["Corporation"]["id"] 		    = $id;
					if($this->Corporation->save($this->data)) {
					$this->Session->setFlash('Corporation information has been updated successfully.');
					$this->redirect($_SERVER["HTTP_REFERER"]);	
					} else {
			...
			switch(trim($status)){
				case "activate":
					for($ctr=0;$ctr<$count;$ctr++){
						$this->Corporation->id = $ids[$ctr];
						$this->Corporation->saveField("status", '1');
					}
					$this->Session->setFlash('Corporation(s) has been activated successfully.');
					break;
				case "deactivate":
					for($ctr=0;$ctr<$count;$ctr++){
						$this->Corporation->id = $ids[$ctr];
						$this->Corporation->saveField("status", '0');
					}
					$this->Session->setFlash('Corporation(s) has been deactivated successfully.');
					break;
				case "delete":
					for($i=0;$i<$count;$i++){
						$this->Corporation->create();
						$this->Corporation->id = $ids[$i];
						
						$this->Corporation->delete($ids[$i]);
						
					}
					$this->Session->setFlash('Corporation(s) has been deleted successfully.');
					break;
			}
		}
		public function update_statusb($status, $ids, $count){
			...
			switch(trim($status)){
				case "activate":
					for($ctr=0;$ctr<$count;$ctr++){
						$this->CorporationBranch->id = $ids[$ctr];
						$this->CorporationBranch->saveField("status", '1');
					}
				...
				case "deactivate":
					for($ctr=0;$ctr<$count;$ctr++){
						$this->CorporationBranch->id = $ids[$ctr];
						$this->CorporationBranch->saveField("status", '0');
					}
					$this->Session->setFlash('Branch(s) has been deactivated successfully.');
					break;
				case "delete":
					for($i=0;$i<$count;$i++){
						$this->CorporationBranch->create();
						$this->CorporationBranch->id = $ids[$i];
						
						$this->CorporationBranch->delete($ids[$i]);
		...
		public function email($mode = null){
			...
			if(isset($_GET['fieldId'])){
				...
				if($row_id > 0){
					$dataCheck = $this->Corporation->find('all', array('conditions'=>array('Corporation.email'=>trim($validateValue), 'Corporation.id !='=>trim($row_id))));
				}else{
					$dataCheck = $this->Corporation->find('all', array('conditions'=>array('Corporation.email'=>trim($validateValue))));
				}
		...
		*/		
		...
		function removePic() {
			...
			if($this->data) {
				...
				$userPic = $this->Corporation->find("first",array("fields"=>array("logo"),"conditions"=>array("Corporation.id"=>$this->data["id"])));
				$picPath = $this->config["upload_path"].$userPic["Club"]["logo"];
				unlink($picPath);
				
				$data["logo"] = null;
				if( $this->Corporation->updateAll($data,array("Corporation.id"=>$this->data["id"])) ) {
		...
		function payments() 
		{
			...
			/* if(!empty($this->params->query["tokenID"]))
			{
				if(preg_match("/QuickHire/",$this->params->query["callerReference"]))
				{
					...
					$this->QuickTask->bindModel(array("belongsTo"=>array("Member"=>array("foreignKey"=>"hirer_id","fields"=>array("tokenId","id","full_name")))));					
					$taskInfo = $this->QuickTask->find("first",array("conditions"=>array("QuickTask.id"=>$callerReference[1])));
					...
					$data 	  =  array(
									...
									'RecipientTokenId' => array(
																'FieldValue' => $taskInfo["QuickTask"]["tokenID"], 
																'FieldType' => 'string'
															),
									'TransactionAmount' => array(
																'FieldValue' =>$taskInfo["QuickTask"]["price"], 
				...
				{
					$this->Task->bindModel(array("belongsTo"=>array("Member"=>array("foreignKey"=>"assigned_user_id","fields"=>array("tokenId","id","full_name")))));
				
					$taskInfo = $this->Task->find("first",array("conditions"=>array("Task.id"=>$this->params->query["callerReference"])));					
					...
					$data =  array(
								...
								'RecipientTokenId' => array(
															'FieldValue' => $taskInfo["Member"]["tokenId"], 'FieldType' => 'string'
														),
								'TransactionAmount' => array(
															'FieldValue' =>$taskInfo["Task"]["price"], 
															'FieldType' => 'Amazon_FPS_Model_Amount'
														),
								'TransactionAmount.CurrencyCode' => array(
																		'FieldValue' =>"USD", 
						...
						if(!empty($repsone)){
							...
							if(!empty($this->params->query["callerReference"])) {
								
								/* $paymentFields["transaction_id"] = "213686Z5Z55Z4132IS7SE53ARCUJ3N5ZEMQ";
								$paymentFields["payment_status"] = "Pending";  
							
								$paymentFields = $this->addQuote($paymentFields);
								$this->Task->updateAll($paymentFields,array("Task.id"=>$this->params->query["callerReference"]));
							}
						}
				}
				//$this->params->query["callerReference"] = 100;
				$this->redirect("/members/payments");
			} */
			...
			/* Fetching all Posted tasks */
				...
				//echo"<pre>";print_r($taskQuickPosted);die();
				/* $taskQuickPosted = $this->QuickTask->find("all",array("conditions"=>array("QuickTask.status"=>"3","QuickTask.hirer_id"=>$this->Session->read("user_id"))));
				
				$i =0;
				foreach($taskQuickPosted as $runing ){
					$taskQuickPosted[$i]["Assigned"] = $this->Member->find("first",array("fields"=>array("full_name","id","logo"),"conditions"=>array("Member.id"=>$runing["QuickTask"]["user_id"])));
					
					$taskQuickPosted[$i]["amazonUrl"] = $this->amazon->generateSenderToken(array("url"=>"members/payments","member_id"=>"QuickHire-".$runing["QuickTask"]["id"],"price"=>$runing["QuickTask"]["price"],"taskName"=>$runing["QuickTask"]["title"],"recipientToken"=>$runing["Member"]["tokenId"]));
...
/* 			$sql   = "UPDATE quick_task_jobs SET video = 'tasks_videos_1361188335.flv' WHERE id = 100";
		...
		/*********Coorporation Branch Start Here*************/
		...
		{			
			...
			if(!empty($this->data)){				
				if( array_key_exists("keyword",$this->data) && !empty($this->data["keyword"]) && ($this->data["keyword"] != "Search by email or Branch Name...") ) {					
					$conditions["OR"] = array(
												"CorporationBranch.branch_name LIKE" => "%".$this->data["keyword"]."%",
												"CorporationBranch.email LIKE" => "%".$this->data["keyword"]."%"
											);
					if( !empty($this->params["named"]["keyword"]) )						
						$keyword = $this->params["named"]["keyword"];					
					
				}else{						
						if( !empty($this->data['CorporationBranch']['statusTop']) ) {
							$action = $this->data['CorporationBranch']['statusTop'];
						}elseif( !empty($this->data['CorporationBranch']['status'])) {
							$action = $this->data['CorporationBranch']['status'];
						}
						
						if(isset($this->data['CorporationBranch']['id']) && count($this->data['CorporationBranch']['id']) > 0) {
							$this->update_statusb(trim($action), $this->data['CorporationBranch']['id'], count($this->data['CorporationBranch']['id']));
						} else {
			...
			if( !empty($this->params["named"]["keyword"]) ) {
				$conditions["OR"] = array(
									"CorporationBranch.branch_name LIKE" => "%".$this->params["named"]["keyword"]."%",
									"CorporationBranch.email LIKE" => "%".$this->params["named"]["keyword"]."%"
								);
				$keyword = $this->params["named"]["keyword"];
			}			
					
			
			$this->paginate = array("conditions"=>$conditions,'limit' => '10', 'order' => array('CorporationBranch.branch_name' => 'ASC'));
			$branches = $this->paginate('CorporationBranch'); //default take the current
			$this->set('branches', $branches);
			$this->set('mode', array('delete'=>'Delete'));
			$this->set('status', $status);
			$this->set('tab', '');
			...
			$this->set('limit', $this->params['request']['paging']['CorporationBranch']['options']['limit']);
			$this->set('page', $this->params['request']['paging']['CorporationBranch']['options']['page']);
		}
			...
			if(!empty($this->data)) {
				...
				if($this->CorporationBranch->validates()) {
						...
						if($this->CorporationBranch->save($this->request->data)) {	
							...
							$this->Session->setFlash('Branch has been created successfully.');
							$this->redirect('/admin/corporations/branch/');
						} else {
							$this->Session->setFlash('Some error has been occured. Please try again.');
						}
				}	
		...
		public function admin_viewbranch(){
			...
			if(!empty($this->params["pass"][0])) {
				$this->set("clubInfo",$this->CorporationBranch->find("first",array("conditions"=>array("CorporationBranch.id"=>$this->params["pass"][0]))));	
			}else{
				$this->redirect($_SERVER["HTTP_REFERER"]);
			}	
		}
		...
		/**********************Manage Corporation Contact ***************************/
		
	public function admin_addcontact(){
			
			$this->set("corporations",$this->Corporation->find('list',array('fields'=>array('Corporation.id','Corporation.company_name'))));
			$branches=$this->CorporationBranch->find('list',array('fields'=>array('CorporationBranch.id','CorporationBranch.branch_name')));
			$this->set('branches',$branches);
			//$this->set("branches",$this->Club->find('list',array('fields'=>array('Club.id','Club.club_name'))));
			...
			if(!empty($this->data)) {
				...
				if($this->CorporationContact->validates()) {
						...
						if($this->CorporationContact->save($this->request->data)) {	
										
							$this->Session->setFlash('Contact has been created successfully.');
							$this->redirect('/admin/corporations/contact/');
						} else {
							$this->Session->setFlash('Some error has been occured. Please try again.');
						}
				}	
	...
	function listbranches()
			{
				...
				if($this->data)
				{
					...
					$branchData = $this->CorporationBranch->find('all',array('fields'=>array('CorporationBranch.id','CorporationBranch.branch_name'),"conditions"=>$condition));
				 	...
				 	$tra = '<select id="CorporationContactBranchId" class="validate[required]" name="data[CorporationContact][branch_id]">';
			...
			if(!empty($this->params["pass"][0])) {
				$this->set("clubInfo",$this->CorporationContact->find("first",array("conditions"=>array("CorporationContact.id"=>$this->params["pass"][0]))));	
			}else{
				$this->redirect($_SERVER["HTTP_REFERER"]);
			}	
		}
		...
		{			
			...
			if(!empty($this->data)){				
				if( array_key_exists("keyword",$this->data) && !empty($this->data["keyword"]) && ($this->data["keyword"] != "Search by email or contact Name...") ) {					
					$conditions["OR"] = array(
												"CorporationContact.title LIKE" => "%".$this->data["keyword"]."%",
												"CorporationContact.email LIKE" => "%".$this->data["keyword"]."%"
											);
					if( !empty($this->params["named"]["keyword"]) )						
						$keyword = $this->params["named"]["keyword"];					
					
				}else{						
						if( !empty($this->data['CorporationContact']['statusTop']) ) {
							$action = $this->data['CorporationContact']['statusTop'];
						}elseif( !empty($this->data['CorporationContact']['status'])) {
							$action = $this->data['CorporationContact']['status'];
						}
						
						if(isset($this->data['CorporationContact']['id']) && count($this->data['CorporationContact']['id']) > 0) {
							$this->update_statusc(trim($action), $this->data['CorporationContact']['id'], count($this->data['CorporationContact']['id']));
						} else {
			...
			if( !empty($this->params["named"]["keyword"]) ) {
				$conditions["OR"] = array(
									"CorporationContact.title LIKE" => "%".$this->params["named"]["keyword"]."%",
									"CorporationContact.email LIKE" => "%".$this->params["named"]["keyword"]."%"
								);
				$keyword = $this->params["named"]["keyword"];
			}			
					
			
			$this->paginate = array("conditions"=>$conditions,'limit' => '10', 'order' => array('CorporationContact.title' => 'ASC'));
			$contacts = $this->paginate('CorporationContact'); //default take the current
			...
			$this->set('limit', $this->params['request']['paging']['CorporationContact']['options']['limit']);
			$this->set('page', $this->params['request']['paging']['CorporationContact']['options']['page']);
		}
		public function update_statusc($status, $ids, $count){
			...
			switch(trim($status)){
				case "activate":
					for($ctr=0;$ctr<$count;$ctr++){
						$this->CorporationContact->id = $ids[$ctr];
						$this->CorporationContact->saveField("status", '1');
					}
				...
				case "deactivate":
					for($ctr=0;$ctr<$count;$ctr++){
						$this->CorporationContact->id = $ids[$ctr];
						$this->CorporationContact->saveField("status", '0');
					}
					$this->Session->setFlash('Contact(s) has been deactivated successfully.');
					break;
				case "delete":
					for($i=0;$i<$count;$i++){
						$this->CorporationContact->create();
						$this->CorporationContact->id = $ids[$i];
						
						$this->CorporationContact->delete($ids[$i]);
			...
			if(!empty($this->data)) {
				...
				if(!empty($this->data["Corporation"]["password"]) && ($this->data["Corporation"]["password"]==$this->data["Corporation"]["con_password"]))
					    {
					   
		
				//$this->Club->set($this->data);
				//if($this->Club->validates()) {
						if( !empty($this->data["Corporation"]["logo"]) ) {
							
							$filename = $this->data["Corporation"]["logo"]["name"];
							$target = $this->config["upload_path"];
							$this->Upload->upload($this->data["Corporation"]["logo"], $target, null, null);
  					        $this->request->data["Corporation"]["logo"] = $this->Upload->result; 					
						}else{	
							
							unset($this->request->data["Corporation"]["logo"]);
							$this->request->data["Corporation"]["logo"] = '';							
					    }
					    ...
					    $this->request->data["Corporation"]["added_date"] 		    = date("Y-m-d h:i:s");
						$this->request->data["Corporation"]["modified_date"] 		    = date("Y-m-d h:i:s");
						$this->request->data["Corporation"]["status"]= 1;
						$this->request->data["Corporation"]["notification_status"]= 1;
						...
						if($this->Corporation->save($this->data)) {
							...
							$this->send_welcome_email_admin($this->request->data["Corporation"]["email"],$this->request->data["Corporation"]["company_name"],$this->request->data["Corporation"]["password"],'',$this->request->data["Corporation"]["username"],$mailcontentfetch['Managemail']['content'],$mailcontentfetch['Managemail']['subject']);
							...
							$data_array=array();
							$data_array[$dbusertype]['username']=$this->request->data["Corporation"]["email"];
							$data_array[$dbusertype]['id']=$this->Corporation->getLastInsertId();
							$data_array[$dbusertype]['company_name']=$this->request->data["Corporation"]["company_name"];
							
							$this->Session->write('USER_ID', $data_array[$dbusertype]['id']);
					$this->Session->write('USER_NAME', $data_array[$dbusertype]['username']);
					$this->Session->write('UNAME', $data_array[$dbusertype]['company_name']);
		...
		$email->emailFormat('html');
				...
				<p>You have registered as Corporation</p>
	...
	function send_welcome_email_admin($emailaddress,$name,$pass,$lname,$username,$mail_con,$mail_sub) {
		...
		$email->emailFormat('html');
				...
				<p>A New Corporation has been registerd on '.$this->config["base_title"].' site. </p>					
		...
		//$email->cc('rprasad@sampatti.com');
		$subtxt = __('New Corporation Registration');
