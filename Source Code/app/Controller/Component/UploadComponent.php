<?php
	...
	class UploadComponent extends Component {
		...
		function upload ($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL) {
			...
			if (!is_null($allowed)) { $this->_allowed = $allowed; } else { $this->_allowed = array('jpg','jpeg','gif','png','JPG','JPEG','GIF','PNG','txt','pdf','doc','docx','xls','xlsx'); }
		 ...
		 $uploadType = $uploadSize[2];
			...
			switch ($type) {
				...
				$ratioY = $maxH / $uploadHeight;
					...
					//echo $maxW.'<br>'.$maxH;die;
					
				 $dstImg = imagecreatetruecolor($maxW, $maxH);
					/*
					  Hack for png images [starts]
                    */					
					    imagealphablending($dstImg, false);
                        imagesavealpha($dstImg, true);
                    /*
					  Hack for png images [ends]
                    */
