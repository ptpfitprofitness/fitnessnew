...
##  Description :		This file contains function for members
## *****************************************************************
 ...
 */
	class MembersController extends AppController {
		...
		public $uses 		= array('Country','Member');
		...
		public function admin_add(){
			...
			if(!empty($this->data)) {
				...
				if($this->Member->validates()) {
						if( !empty($this->data["Member"]["picture"]) ) {
							$filename = $this->data["Member"]["picture"]["name"];
							$target = $this->config["upload_path"];
							$this->Upload->upload($this->data["Member"]["picture"], $target, null, null);
							$this->request->data["Member"]["picture"] = $this->Upload->result;  					
						}else{	
							
							unset($this->request->data["Member"]["picture"]);
							$this->request->data["Member"]["picture"] = '';							
					    }	
						if($this->Member->save($this->data)) {				
							$this->Session->setFlash('Customer has been created successfully.');
		...
		public function admin_view(){
			...
			if(!empty($this->params["pass"][0])) {
				$this->set("memberInfo",$this->Member->find("first",array("conditions"=>array("Member.id"=>$this->params["pass"][0]))));	
			}else{
		...
		public function admin_edit($id = null)
		{
			...
			$this->Member->set($this->data);
			$this->Member->id = $this->data['Member']['id'];		
			
							
			if($this->Member->validates()) {
				
				if(!empty($this->request->data["Member"]["picture"]["name"]))
				{
					$filename = $this->request->data["Member"]["picture"]["name"];
					$target = $this->config["upload_path"];
					$this->Upload->upload($this->data["Member"]["picture"], $target, null, null);
  					$this->request->data["Member"]["picture"] = $this->Upload->result; 
  					$picPath = $this->config["upload_path"].$this->request->data["Member"]["old_image"];
					@unlink($picPath);
				}else{	
					if(!empty($this->request->data["Member"]["old_image"])){
						$this->request->data["Member"]["picture"] = $this->request->data["Member"]["old_image"];			
					}
					else{
						$this->request->data["Member"]["picture"] = "";
					}
				...
				if($this->Member->save($this->data)) {
					$this->Session->setFlash('Customer information has been updated successfully.');
					$this->redirect('/admin/members/');
				} else {
					$this->Session->setFlash('Some error has been occured. Please try again.');
				}
				...
				if(is_numeric($id) && $id > 0) {
						...
						$this->Session->setFlash('Invalid member id.');
						$this->redirect('/admin/members/');
				}
			}	
		}
		...
		{			
			...
			if(!empty($this->data)){				
				if( array_key_exists("keyword",$this->data) && !empty($this->data["keyword"]) && ($this->data["keyword"] != "Search by email or UserName...") ) {					
					$conditions["OR"] = array(
												"Member.username LIKE" => "%".$this->data["keyword"]."%",
												"Member.email LIKE" => "%".$this->data["keyword"]."%"
											);
					if( !empty($this->params["named"]["keyword"]) )						
						$keyword = $this->params["named"]["keyword"];					
					
				}else{						
						if( !empty($this->data['Member']['statusTop']) ) {
							$action = $this->data['Member']['statusTop'];
						}elseif( !empty($this->data['Member']['status'])) {
							$action = $this->data['Member']['status'];
						}
						
						if(isset($this->data['Member']['id']) && count($this->data['Member']['id']) > 0) {
							$this->update_status(trim($action), $this->data['Member']['id'], count($this->data['Member']['id']));
						} else {
							
							
							if(isset($this->data["submit"]) && isset($this->data["keyword"]) && ($this->data["keyword"]=='' || $this->data["keyword"]=='Search by email or UserName...') && $this->data["submit"]=='Search'){
			...
			if( !empty($this->params["named"]["keyword"]) ) {
				$conditions["OR"] = array(
									"Member.username LIKE" => "%".$this->params["named"]["keyword"]."%",
									"Member.email LIKE" => "%".$this->params["named"]["keyword"]."%"
								);
				$keyword = $this->params["named"]["keyword"];
			}			
					
			
			$this->paginate = array("conditions"=>$conditions,'limit' => '10', 'order' => array('Member.username' => 'ASC'));
		...
		public function update_status($status, $ids, $count){
			...
			switch(trim($status)){
				case "activate":
					...
					$this->Session->setFlash('Customer(s) has been activated successfully.');
					break;
				case "deactivate":
					for($ctr=0;$ctr<$count;$ctr++){
						$this->Member->id = $ids[$ctr];
					...
					$this->Session->setFlash('Customer(s) has been deactivated successfully.');
					break;
				case "delete":
					for($i=0;$i<$count;$i++){
						$this->Member->create();
						...
						$this->Member->delete($ids[$i]);
						
					}
					$this->Session->setFlash('Customer(s) has been deleted successfully.');
					break;
			}
		}
		...
		public function email($mode = null){
			...
			if(isset($_GET['fieldId'])){
				...
				if($row_id > 0){
					$dataCheck = $this->Member->find('all', array('conditions'=>array('Member.email'=>trim($validateValue), 'Member.id !='=>trim($row_id))));
				}else{
					$dataCheck = $this->Member->find('all', array('conditions'=>array('Member.email'=>trim($validateValue))));
				}
		...
		*/		
		...
		function removePic() {
			...
			if($this->data) {
				...
				$userPic = $this->Member->find("first",array("fields"=>array("picture"),"conditions"=>array("Member.id"=>$this->data["id"])));
				$picPath = $this->config["upload_path"].$userPic["Member"]["picture"];
				unlink($picPath);
				
				$data["picture"] = null;
				if( $this->Member->updateAll($data,array("Member.id"=>$this->data["id"])) ) {
